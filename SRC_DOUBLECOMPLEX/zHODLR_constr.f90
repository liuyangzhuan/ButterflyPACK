...
subroutine z_HODLR_construction(ho_bf1,option,stats,msh,ker,element_Zmn,ptree)
		...
		! do ii =Bidxs,Bidxs
			...
			if(ptree%MyID >=ptree%pgrp(ho_bf1%levels(level_c)%BP(ii)%pgno)%head .and. ptree%MyID <=ptree%pgrp(ho_bf1%levels(level_c)%BP(ii)%pgno)%tail)then
				if (level_c/=ho_bf1%Maxlevel+1) then
					...
					! if(mod(ii,2)==1)then
						call z_Bplus_compress_N15(ho_bf1%levels(level_c)%BP(ii),option,rtemp,stats,msh,ker,element_Zmn,ptree)				
					! else
					...
					! if(level==option%level_check)then
						! ! call z_Bplus_randomized_Exact_test(ho_bf1%levels(level_c)%BP(ii))
						...
						! call z_Bplus_randomized_constr(level_butterfly,ho_bf1%levels(level_c)%BP(ii),ho_bf1%levels(level_c)%BP(ii),rank0_inner,rankrate_inner,Bplus_block_MVP_Exact_dat,rank0_outter,rankrate_outter,Bplus_block_MVP_Outter_Exact_dat,error,'Exact',option,stats,ptree)
...
end subroutine z_HODLR_construction
