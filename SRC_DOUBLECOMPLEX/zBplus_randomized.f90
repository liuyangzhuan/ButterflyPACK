...
module z_Bplus_randomized
! use Utilites_randomized

use z_misc
use z_HODLR_Utilities
	...
	type is (blockplus)
		...
		! type is (hobf)	
			...
			if(option%TwoLayerOnly==1)then
				...
				! bb = boxindex(1)
				! blocks => z_Bplus_randomized_constr(1)%LL(ll)%matrices_block(bb)
...
subroutine z_Bplus_randomized_constr(level_butterfly,bplus_o,operand,rank0_inner,rankrate_inner,blackbox_MVP_dat_inner,rank0_outter,rankrate_outter,blackbox_MVP_dat_outter,error_inout,strings,option,stats,ptree)
   ...
   implicit none
	...
	type(Hoption)::option
	type(Hstat)::stats
	type(blockplus) :: z_Bplus_randomized
	procedure(BF_MVP_blk)::blackbox_MVP_dat_inner,blackbox_MVP_dat_outter
	type(proctree)::ptree
	error_inout=0
	! Memory = 0	
	call assert(bplus_o%Lplus>=2,'this is not a multi Bplus in z_Bplus_randomized')
	
	call Bplus_Init_FromInput(bplus_o,z_Bplus_randomized)
	
	! write(*,*)'hhhhh1',z_Bplus_randomized%LL(1)%matrices_block(1)%level
	n1 = OMP_get_wtime()

	rankthusfar = 0	
	do bb =1,z_Bplus_randomized%LL(2)%Nbound
		! ! write(*,*)bb,z_Bplus_randomized%LL(2)%Nbound,'dddd'

		
		! rank0_inner = ho_bf1%levels(level_c)%BP(2*rowblock-1)%LL(2)%rankmax
		! rankrate_inner = 2.0d0
		block_o => z_Bplus_randomized%LL(2)%matrices_block(bb)
		! ho_bf1%ind_lv = level_c
		! ho_bf1%ind_bk = rowblock
		
		call Bplus_MultiLrandomized_Onesubblock(rank0_inner,rankrate_inner,rankthusfar,block_o,operand,blackbox_MVP_dat_inner,error,strings,option,stats,ptree,z_Bplus_randomized)		
	...
	! rate_outter=1.2d0
	call BF_randomized(level_butterfly,rank0_outter,rankrate_outter,z_Bplus_randomized%LL(1)%matrices_block(1),operand,blackbox_MVP_dat_outter,error,'Outter',option,stats,ptree,z_Bplus_randomized)
	error_inout = max(error_inout, error)

	
	call delete_Bplus(bplus_o)
	call copy_delete_Bplus(z_Bplus_randomized,bplus_o,Memory)
	! deallocate(z_Bplus_randomized)
...
end subroutine z_Bplus_randomized_constr
...
subroutine Bplus_Init_FromInput(Bplus,z_Bplus_randomized)
	use z_misc
    use z_HODLR_DEFS
	...
	type(blockplus)::Bplus,z_Bplus_randomized
	integer seed_myid(2)
	integer time(8)
	integer ll,bb
	...
	! allocate(z_Bplus_randomized)
	
	z_Bplus_randomized%level = Bplus%level
	z_Bplus_randomized%col_group = Bplus%col_group
	z_Bplus_randomized%row_group = Bplus%row_group
	z_Bplus_randomized%Lplus = Bplus%Lplus
	z_Bplus_randomized%boundary = Bplus%boundary
	
	allocate(z_Bplus_randomized%LL(LplusMax))
	

	! z_Bplus_randomized%LL(1)%Nbound = 1
	! allocate(z_Bplus_randomized%LL(1)%matrices_block(1))
	! z_Bplus_randomized%LL(1)%matrices_block(1)%level = z_Bplus_randomized%level
	! z_Bplus_randomized%LL(1)%matrices_block(1)%col_group = z_Bplus_randomized%col_group
	! z_Bplus_randomized%LL(1)%matrices_block(1)%row_group = z_Bplus_randomized%row_group			
	! z_Bplus_randomized%LL(1)%matrices_block(1)%style = Bplus%LL(1)%matrices_block(1)%style
	allocate(z_Bplus_randomized%LL(1)%boundary_map(1))
	z_Bplus_randomized%LL(1)%boundary_map(1) = Bplus%LL(1)%boundary_map(1)
	
	do ll=1,LplusMax
	z_Bplus_randomized%LL(ll)%Nbound = 0
	end do	
	
	do ll=1,LplusMax-1
		if(Bplus%LL(ll)%Nbound>0)then

			z_Bplus_randomized%LL(ll)%rankmax = Bplus%LL(ll)%rankmax
			z_Bplus_randomized%LL(ll)%Nbound = Bplus%LL(ll)%Nbound
			allocate(z_Bplus_randomized%LL(ll)%matrices_block(z_Bplus_randomized%LL(ll)%Nbound))
			
			do bb =1,z_Bplus_randomized%LL(ll)%Nbound
				z_Bplus_randomized%LL(ll)%matrices_block(bb)%row_group = Bplus%LL(ll)%matrices_block(bb)%row_group
				z_Bplus_randomized%LL(ll)%matrices_block(bb)%col_group = Bplus%LL(ll)%matrices_block(bb)%col_group
				z_Bplus_randomized%LL(ll)%matrices_block(bb)%style = Bplus%LL(ll)%matrices_block(bb)%style
				z_Bplus_randomized%LL(ll)%matrices_block(bb)%level = Bplus%LL(ll)%matrices_block(bb)%level
			end do
			
			
			if(Bplus%LL(ll+1)%Nbound==0)then		
				z_Bplus_randomized%LL(ll+1)%Nbound=0
			else 
				...
				allocate(z_Bplus_randomized%LL(ll+1)%boundary_map(Nboundall))
				! write(*,*)shape(Bplus%LL(ll+1)%boundary_map),shape(z_Bplus_randomized%LL(ll+1)%boundary_map),'didi',ll
				
				! write(*,*)'gali',ll,Nboundall,shape(Bplus%LL(ll+1)%boundary_map)
				z_Bplus_randomized%LL(ll+1)%boundary_map = Bplus%LL(ll+1)%boundary_map
			end if
		else 
			exit
		end if
