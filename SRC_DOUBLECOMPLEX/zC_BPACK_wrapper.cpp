...
void z_c_bpack_set_option_from_command_line(int argc, const char* const* cargv,F2Cptr option0) {
	...
	while ((c = getopt_long_only
		...
		std::istringstream iss(optarg);
		iss >> opt_i;
		z_c_bpack_set_I_option(&option0, "trans_invariant", opt_i);
		} break;
			case 41: {
			std::istringstream iss(optarg);
			iss >> opt_i;
			z_c_bpack_set_I_option(&option0, "htensor_mvp_level_batch", opt_i);
			} break;
		case 36: {
		std::istringstream iss(optarg);
		iss >> opt_i;
		...
		std::istringstream iss(optarg);
		iss >> opt_i;
		z_c_bpack_set_I_option(&option0, "iter_solver", opt_i);
		} break;
		default: break;
		}
	}
...
void z_c_bpack_vector_global2local(F2Cptr* ptree, F2Cptr* msh, int* nvec_p, _Complex double    *b_global, _Complex double    *b)
{
	...
	MPI_Comm comm;
	int fcomm;
	z_c_bpack_get_comm(ptree, &fcomm);
	comm = MPI_Comm_f2c((MPI_Fint)fcomm);

	MPI_Comm_size(comm, &size);
    MPI_Comm_rank(comm, &iam);
	MPI_Bcast( &nvec, 1, MPI_INT, 0, comm);
	z_c_bpack_localindices(msh, &idxs, &m_loc, &m);
	fst_row = idxs-1;

	{
	_Complex double * b_global_permed=NULL;
	_Complex double * btmp= (_Complex double *)malloc(m_loc * nvec*sizeof(_Complex double ));
	int *counts = NULL, *displs = NULL;

	if (iam == 0) {
		counts = (int *) malloc(size*sizeof(int));
		displs = (int *) malloc(size*sizeof(int));
		b_global_permed= (_Complex double *)malloc(m * nvec*sizeof(_Complex double ));
		for (int j = 0; j < nvec; ++j) {
			for (int i = 0; i < m; ++i) {
				int i_old= i+1;
				int i_new;
				z_c_bpack_old2new(msh,&i_old,&i_new);
	...
	MPI_Scatterv(b_global_permed, counts, displs, MPI_C_DOUBLE_COMPLEX ,  btmp, m_loc * nvec, MPI_C_DOUBLE_COMPLEX , 0, comm);
...
void z_c_bpack_vector_local2global(F2Cptr* ptree, F2Cptr* msh, int* nvec_p, _Complex double    *b, _Complex double    *b_global)
{
	int idxs, m_loc, fst_row, m, m_loc1, fst_row1;
	int nvec = *nvec_p;
    int iam, size;
	...
	{
	_Complex double * b_global_permed;
	_Complex double * btmp= (_Complex double *)malloc(m_loc * nvec*sizeof(_Complex double ));
	for (int j = 0; j < nvec; ++j)
	{
		for (int i = 0; i < m_loc; ++i)
	...
	MPI_Gather(&m_loc1, 1, MPI_INT, counts, 1, MPI_INT, 0, comm);
	MPI_Gather(&fst_row1, 1, MPI_INT, displs, 1, MPI_INT, 0, comm);
	MPI_Gatherv(btmp, m_loc * nvec, MPI_C_DOUBLE_COMPLEX , b_global_permed, counts, displs, MPI_C_DOUBLE_COMPLEX , 0, comm);
	if (iam == 0) {
		for (int j = 0; j < nvec; ++j) {
			for (int i = 0; i < m; ++i) {
				int i_old= i+1;
...
void z_c_bpack_md_vector_local2global(F2Cptr* ptree, F2Cptr* msh, int* ndim_p, int* nvec_p, _Complex double    *b, _Complex double    *b_global)
{
	...
	MPI_Gatherv(b, m_loc1, MPI_C_DOUBLE_COMPLEX , b_global, counts, displs, MPI_C_DOUBLE_COMPLEX , 0, comm);
	...
	if (iam == 0) {
		...
		for (int s=0;s<size;s++){
			...
			for (int i=0; i<m_loc_1vec; i++){
				...
				int i_new_md[Ndim];
				z_c_bpack_singleindex_to_multiindex(&Ndim,&(m_locs[s*Ndim]),&i_new_scalar,i_new_md);
				for (int j=0;j<Ndim;j++){
					i_new_md[j]=i_new_md[j]+idxss[s*Ndim+j]-1;
				}
				z_c_bpack_multiindex_to_singleindex(&Ndim,m,&i_new_scalar,i_new_md);
		...
		for (int i = 0; i < m_scalar; ++i) {
			...
			int i_old_md[Ndim];
			int i_new_md[Ndim];
			z_c_bpack_singleindex_to_multiindex(&Ndim,m,&i_old_scalar,i_old_md);
			z_c_bpack_md_old2new(&Ndim, msh, i_old_md, i_new_md);
			z_c_bpack_multiindex_to_singleindex(&Ndim,m,&i_new_scalar,i_new_md);
			for (int nth=0; nth<nvec; nth++){
...
void z_c_bpack_construct_init(int* Npo, int* Ndim, double* Locations, int* nns, int* nlevel, int* tree, int* perms, int* Npo_loc, F2Cptr* bmat, F2Cptr* option,F2Cptr* stats,F2Cptr* msh,F2Cptr* ker,F2Cptr* ptree, void (*C_FuncDistmn)(int*, int*, double*,C2Fptr), void (*C_FuncNearFar)(int*, int*, int*,C2Fptr), C2Fptr C_QuantApp){
  ...
  if(format==7){
	...
	z_c_bpack_construct_init_fortran(Npo, Ndim, Locations, nns, nlevel, tree, perms, Npo_loc, bmat, option, stats, msh, ker, ptree, C_FuncDistmn, C_FuncNearFar, C_QuantApp);
  }
}
...
void z_c_bpack_construct_element_compute(F2Cptr* bmat, F2Cptr* option,F2Cptr* stats,F2Cptr* msh,F2Cptr* ker,F2Cptr* ptree, void (*C_FuncZmn)(int*, int*, _Complex double *,C2Fptr),void (*C_FuncZmnBlock)(int*, int*, int*, int64_t*, int*, int*, _Complex double *, int*, int*, int*, int*, int*, C2Fptr), C2Fptr C_QuantApp){
  ...
  if(format==7){
	...
	z_c_bpack_construct_element_compute_fortran(bmat, option, stats, msh, ker, ptree, C_FuncZmn, C_FuncZmnBlock, C_QuantApp);
  }
}
...
void z_c_bpack_factor(F2Cptr*bmat, F2Cptr*option, F2Cptr*stats, F2Cptr*ptree, F2Cptr*msh){
  ...
  if(format==7){
	...
	z_c_bpack_factor_fortran(bmat, option, stats, ptree, msh);
  }
}
...
void z_c_bpack_solve(_Complex double *x, _Complex double *b, int*Nloc, int*Nrhs, F2Cptr*bmat, F2Cptr*option, F2Cptr*stats, F2Cptr*ptree){
  ...
  if(format==7){
	...
	z_c_bpack_solve_fortran(x, b, Nloc, Nrhs, bmat, option, stats, ptree);
  }
}
...
void z_c_bpack_mult(char const * trans, _Complex double  const * xin, _Complex double * xout, int* Ninloc, int* Noutloc, int* Ncol, F2Cptr* bmat,F2Cptr* option,F2Cptr* stats,F2Cptr* ptree){
  ...
  if(format==7){
	...
	z_c_bpack_mult_fortran(trans, xin, xout, Ninloc, Noutloc, Ncol, bmat, option, stats, ptree);
  }
}
...
void z_c_bpack_logdet(_Complex double * phase, double* logabsdet, F2Cptr* option, F2Cptr* bmat){
  ...
  if(format==7){
	...
	z_c_bpack_logdet_fortran(phase, logabsdet, option, bmat);
  }
}