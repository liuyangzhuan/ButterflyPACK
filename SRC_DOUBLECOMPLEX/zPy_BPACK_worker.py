...
while True:
    ...
    if(flag=="init"):
        ...
        sp = zPy_BPACK_wrapper.z_py_bpack_load()
        ####################### initialization
        pyobjs[fid] = ctypes.c_void_p()
        maxrank = ctypes.c_int(0)
        sp.z_py_bpack_init_compute(
        ...
        if(rank==0):
            print("maxrank from z_py_bpack_init_compute:", maxrank.value)

    elif(flag=="factor"):
        ####################### factor
        sp.z_py_bpack_factor(
            ctypes.byref(pyobjs[fid]),            # void **pyobj
        )
    elif(flag=="solve"):
        ####################### solve
        ...
        sp.z_py_bpack_solve(
            ctypes.byref(pyobjs[fid]),            # void **pyobj
            nrhs,                           # int nrhs
            xb.ctypes.data_as(ctypes.POINTER(ctypes.c_double*2)),
        )
        if rank == 0:
        ...
        ####################### log-determinant
        sign = ctypes.c_double*2(1)
        logdet = ctypes.c_double(0.0)
        sp.z_py_bpack_logdet(
            ctypes.byref(pyobjs[fid]),            # void **pyobj
        ...
        ####################### free stuff
        sp.z_py_bpack_free(ctypes.byref(pyobjs[fid]))
    elif(flag=="terminate"):
        sp.z_py_bpack_terminate()
        break
