...
while True:
    ...
    if(flag=="init"):
        ...
        sp = sPy_BPACK_wrapper.s_py_bpack_load()
        ####################### initialization
        pyobjs[fid] = ctypes.c_void_p()
        maxrank = ctypes.c_int(0)
        sp.s_py_bpack_init_compute(
        ...
        if(rank==0):
            print("maxrank from s_py_bpack_init_compute:", maxrank.value)

    elif(flag=="factor"):
        ####################### factor
        sp.s_py_bpack_factor(
            ctypes.byref(pyobjs[fid]),            # void **pyobj
        )
    elif(flag=="solve"):
        ####################### solve
        ...
        nrhs=-1
        xb = np.random.rand(1).astype(np.float32)
        if rank == 0:
            with open(f"{DATA_FILE}.{fid}", "rb") as f:
        ...
        ####################### log-determinant
        sign = ctypes.c_float(1)
        logdet = ctypes.c_float(0.0)
        sp.s_py_bpack_logdet(
            ctypes.byref(pyobjs[fid]),            # void **pyobj
        ...
        ####################### free stuff
        sp.s_py_bpack_free(ctypes.byref(pyobjs[fid]))
    elif(flag=="terminate"):
        sp.s_py_bpack_terminate()
        break
