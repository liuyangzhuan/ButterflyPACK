...
!> @file s_BPACK_wrapper.f90
!> @brief C++ interfaces for the high-level Fortran subroutines using iso_c_binding

#include "sButterflyPACK_config.fi"
module s_BPACK_wrapper
   use s_BPACK_DEFS
   use s_BPACK_structure
   use s_BPACK_factor
   use s_BPACK_constr
   use s_BPACK_randomMVP
#ifdef HAVE_OPENMP
   use omp_lib
#endif
   use s_MISC_Utilities
   use s_BPACK_Solve_Mul
   use s_Bplus_Utilities, only: BF_Switchlevel
   use iso_c_binding
...
!>**** C interface of process tree construction
   ...
   subroutine C_BPACK_Createptree(nmpi, groupmembers, MPIcomm, ptree_Cptr) bind(c, name="s_c_bpack_createptree")
      implicit none
      integer nmpi
      integer MPIcomm
   ...
   subroutine C_BPACK_Get_Comm(ptree_Cptr, fcomm) bind(c, name="s_c_bpack_get_comm")
   use iso_c_binding
   implicit none
   type(c_ptr) :: ptree_Cptr
   integer(c_int)     :: fcomm
...
!>**** C interface of initializing statistics
   ...
   subroutine C_BPACK_Createstats(stats_Cptr) bind(c, name="s_c_bpack_createstats")
      implicit none
...
!>**** C interface of getting one entry in stats
   ...
   subroutine C_BPACK_Getstats(stats_Cptr, nam, val_d) bind(c, name="s_c_bpack_getstats")
      implicit none
      real(kind=8)::val_d
...
!>**** C interface of printing statistics
   !> @param stats_Cptr: the structure containing statistics
   !> @param ptree_Cptr: the structure containing process tree
   subroutine C_BPACK_Printstats(stats_Cptr, ptree_Cptr) bind(c, name="s_c_bpack_printstats")
      implicit none
      type(c_ptr) :: stats_Cptr
      type(c_ptr) :: ptree_Cptr
      type(Hstat), pointer::stats
...
!>**** C interface of printing the structure, size, and memory of each admissible block of the hierarchical matrix
   ...
   subroutine C_BPACK_PrintStructure(bmat_Cptr,inverse, option_Cptr, stats_Cptr, ptree_Cptr) bind(c, name="s_c_bpack_printstructure")
      implicit none
      type(c_ptr) :: stats_Cptr
      type(c_ptr) :: ptree_Cptr
...
!>**** C interface of initializing option
   !> @param option_Cptr: the structure containing option
   subroutine C_BPACK_Createoption(option_Cptr) bind(c, name="s_c_bpack_createoption")
      implicit none
      type(c_ptr) :: option_Cptr
      type(Hoption), pointer::option
...
!>**** C interface of copy option
   ...
   subroutine C_BPACK_Copyoption(option_Cptr, option_Cptr1) bind(c, name="s_c_bpack_copyoption")
      implicit none
...
!>**** C interface of printing option
   !> @param option_Cptr: the structure containing option
   !> @param ptree_Cptr: the structure containing process tree
   subroutine C_BPACK_Printoption(option_Cptr, ptree_Cptr) bind(c, name="s_c_bpack_printoption")
      implicit none
      type(c_ptr) :: option_Cptr
      type(c_ptr):: ptree_Cptr
      type(Hoption), pointer::option
...
!>**** C interface of getting one entry in option, always returning double
   ...
   subroutine C_BPACK_Getoption(option_Cptr, nam, val_d) bind(c, name="s_c_bpack_getoption")
      implicit none
      real(kind=8)::val_d
      character(kind=c_char, len=1) :: nam(*)
      character(kind=c_char) :: tmpc
...
!>**** C interface of set one entry in option
   ...
   subroutine C_BPACK_Setoption(option_Cptr, nam, val_Cptr) bind(c, name="s_c_bpack_setoption")
      implicit none
      type(c_ptr) :: option_Cptr
      character(kind=c_char, len=1) :: nam(*)
      type(Hoption), pointer::option
...
!>**** C interface of matrix construction
   ...
   subroutine C_BPACK_Construct_Element_Compute(bmat_Cptr, option_Cptr, stats_Cptr, msh_Cptr, ker_Cptr, ptree_Cptr, C_FuncZmn, C_FuncZmnBlock, C_QuantApp) bind(c, name="s_c_bpack_construct_element_compute_fortran")
      implicit none
...
!>**** C interface of tensor construction
   ...
   !> @param C_QuantApp: the C_pointer to user-defined quantities required to for entry evaluation,sampling,distance and compressibility test (in)
   subroutine C_BPACK_MD_Construct_Element_Compute(Ndim,bmat_Cptr, option_Cptr, stats_Cptr, msh_Cptr, ker_Cptr, ptree_Cptr, C_FuncZmn_MD, C_FuncZmnBlock_MD, C_QuantApp) bind(c, name="s_c_bpack_md_construct_element_compute")
      implicit none
...
!>**** C interface of matrix construction via entry evaluation
   ...
   !> @param C_QuantApp: the C_pointer to user-defined quantities required to for entry evaluation,sampling,distance and compressibility test (in)
   subroutine C_BPACK_Construct_Init(N, Ndim, Locations, nns, nlevel, tree, Permutation, N_loc, bmat_Cptr, option_Cptr, stats_Cptr, msh_Cptr, ker_Cptr, ptree_Cptr, C_FuncDistmn, C_FuncNearFar, C_QuantApp) bind(c, name="s_c_bpack_construct_init_fortran")
      implicit none
      integer N, Ndim
...
!>**** C interface of multi-dimensional BF construction via entry evaluation
   ...
   !> @param C_QuantApp: the C_pointer to user-defined quantities required to for entry evaluation,sampling,distance and compressibility test (in)
   subroutine C_BPACK_MD_Construct_Init(Ns, Nmax, Ndim, Locations, Permutation, N_loc, bmat_Cptr, option_Cptr, stats_Cptr, msh_Cptr, ker_Cptr, ptree_Cptr, C_FuncNearFar_MD, C_QuantApp) bind(c, name="s_c_bpack_md_construct_init")
      implicit none
      integer Ndim,Nmax
      integer Ns(Ndim)
      real(kind=8) Locations(*)
...
!>**** C interface of converting from new,local index to old, global index, the indexs start from 1
   ...
   !> @param msh_Cptr: the structure containing points and ordering information
   subroutine C_BPACK_New2Old(msh_Cptr, newidx_loc, oldidx) bind(c, name="s_c_bpack_new2old")
      implicit none
...
!>**** C interface of converting from old, global index to new, global index, the indexs start from 1
   ...
   !> @param msh_Cptr: the structure containing points and ordering information
   subroutine C_BPACK_Old2New(msh_Cptr, oldidx, newidx) bind(c, name="s_c_bpack_old2new")
      implicit none
...
!>**** C interface of getting the start index and local (and global) number of indices (in the permuted order)
   !> @param msh_Cptr: the structure containing points and ordering information
   subroutine C_BPACK_LocalIndices(msh_Cptr, idxs, Nlocal, Nglobal) bind(c, name="s_c_bpack_localindices")
      implicit none
...
!>**** C interface of converting from new,local index to old, global index, the indexs start from 1. Both newidx_loc and oldidx are Ndim dimensional.
   ...
   !> @param oldidx: old, global index, from 1 to N (out)
   !> @param msh_Cptr: the structure containing points and ordering information
   subroutine C_BPACK_MD_New2Old(Ndim, msh_Cptr, newidx_loc, oldidx) bind(c, name="s_c_bpack_md_new2old")
      implicit none
      integer Ndim,dim_i
...
!>**** C interface of converting from new,global index to old, global index. Both newidx_glo and oldidx are Ndim dimensional.
   subroutine C_BPACK_MD_Global_New2Old(Ndim, msh_Cptr, newidx_glo, oldidx) bind(c, name="s_c_bpack_md_global_new2old")
      implicit none
      integer Ndim,dim_i
...
!>**** C interface of converting from old, global index to new, global index, the indexs start from 1
   ...
   !> @param oldidx: old, global index, from 1 to N (in)
   !> @param msh_Cptr: the structure containing points and ordering information
   subroutine C_BPACK_MD_Old2New(Ndim, msh_Cptr, oldidx, newidx) bind(c, name="s_c_bpack_md_old2new")
      implicit none
      integer Ndim,dim_i
...
!>**** C interface of getting the start index and local (and global) number of indices (in the permuted order)
   !> @param msh_Cptr: the structure containing points and ordering information
   subroutine C_BPACK_MD_LocalIndices(Ndim,msh_Cptr, idxs, Nlocal, Nglobal) bind(c, name="s_c_bpack_md_localindices")
      implicit none
      integer Ndim,dim_i
...
!>**** C interface of Fortran subroutine SingleIndexToMultiIndex for converting single index to multi-index
   ...
   subroutine C_SingleIndexToMultiIndex(Ndim,dims, single_index_in, multi_index)bind(c, name="s_c_bpack_singleindex_to_multiindex")
      implicit none
      integer:: Ndim
      integer:: dims(Ndim)
...
!>**** C interface of Fortran subroutine MultiIndexToSingleIndex for converting multi-index to single index
   ...
   !> @param multi_index(Ndim): multi_index
   !>**** convert single index to multi-index, assuming first index is the fastest
   subroutine C_MultiIndexToSingleIndex(Ndim,dims, single_index_in, multi_index)bind(c, name="s_c_bpack_multiindex_to_singleindex")
      implicit none
      integer:: Ndim
      integer:: dims(Ndim)
      integer :: single_index_in, multi_index(Ndim)
...
!>**** C interface of converting from new,local index to old, global index, the indexs start from 1
   ...
   subroutine C_BF_New2Old_Row(mshr_Cptr, newidx_loc, oldidx) bind(c, name="s_c_bf_new2old_row")
      implicit none
      integer newidx_loc,oldidx
      type(c_ptr) :: msh_Cptr
...
!>**** C interface of converting from new,local index to old, global index, the indexs start from 1
   ...
   subroutine C_BF_New2Old_Col(mshc_Cptr, newidx_loc, oldidx) bind(c, name="s_c_bf_new2old_col")
      implicit none
      integer newidx_loc,oldidx
      type(c_ptr) :: msh_Cptr
...
!>**** C interface of matrix construction via entry evaluation and using it for gram distance
   ...
   !> @param C_FuncZmnBlock: the C_pointer to user-provided function to sample a list of intersections of entries of the matrix
   !> @param C_QuantApp: the C_pointer to user-defined quantities required to for entry evaluation,sampling,distance and compressibility test (in)
   subroutine C_BPACK_Construct_Init_Gram(N, Ndim, Locations, nns, nlevel, tree, Permutation, N_loc, bmat_Cptr, option_Cptr, stats_Cptr, msh_Cptr, ker_Cptr, ptree_Cptr, C_FuncZmn, C_FuncZmnBlock, C_QuantApp) bind(c, name="s_c_bpack_construct_init_gram")
      implicit none
      integer N, Ndim
      real(kind=8) Locations(*)
      integer nns(*)
...
!>**** C interface of matrix construction via blackbox matvec
   ...
   !> @param C_QuantApp: the C_pointer to user-defined quantities required to for entry evaluation,sampling,distance and compressibility test (in)
   subroutine C_BPACK_Construct_Matvec_Compute(bmat_Cptr, option_Cptr, stats_Cptr, msh_Cptr, ker_Cptr, ptree_Cptr, C_FuncHMatVec, C_QuantApp) bind(c, name="s_c_bpack_construct_matvec_compute")
      implicit none
...
!>**** C interface of BF construction via blackbox matvec or entry extraction
   ...
   !> @param C_FuncNearFar: the C_pointer to user-provided function to determine whether a block (in permuted order) is compressible or not
   !> @param C_QuantApp: the C_pointer to user-defined quantities required to for entry evaluation,sampling,distance and compressibility test (in)
   subroutine C_BF_Construct_Init(M, N, M_loc, N_loc, nnsr, nnsc, mshr_Cptr, mshc_Cptr, bf_Cptr, option_Cptr, stats_Cptr, msh_Cptr, ker_Cptr, ptree_Cptr, C_FuncDistmn, C_FuncNearFar, C_QuantApp) bind(c, name="s_c_bf_construct_init")
      implicit none
      integer M, N
...
!>**** C interface of BF construction via blackbox matvec
   ...
   !> @param C_QuantApp: the C_pointer to user-defined quantities required to for entry evaluation,sampling,distance and compressibility test (in)
   subroutine C_BF_Construct_Matvec_Compute(bf_Cptr, option_Cptr, stats_Cptr, msh_Cptr, ker_Cptr, ptree_Cptr, C_FuncBMatVec, C_QuantApp) bind(c, name="s_c_bf_construct_matvec_compute")
      implicit none
...
!>**** C interface of BF construction via entry extraction
   ...
   !> @param C_QuantApp: the C_pointer to user-defined quantities required to for entry evaluation,sampling,distance and compressibility test (in)
   subroutine C_BF_Construct_Element_Compute(bf_Cptr, option_Cptr, stats_Cptr, msh_Cptr, ker_Cptr, ptree_Cptr, C_FuncZmn, C_FuncZmnBlock, C_QuantApp) bind(c, name="s_c_bf_construct_element_compute")
      implicit none
...
!>**** C interface of BPACK factorization
   ...
   !> @param ptree_Cptr: the structure containing process tree
   !> @param msh_Cptr: the structure containing points and ordering information (in)
   subroutine C_BPACK_Factor(bmat_Cptr, option_Cptr, stats_Cptr, ptree_Cptr, msh_Cptr) bind(c, name="s_c_bpack_factor_fortran")
      implicit none
...
!>**** C interface of BPACK solve
   ...
   !> @param stats_Cptr: the structure containing statistics
   !> @param ptree_Cptr: the structure containing process tree
   subroutine C_BPACK_Solve(x, b, Nloc, Nrhs, bmat_Cptr, option_Cptr, stats_Cptr, ptree_Cptr) bind(c, name="s_c_bpack_solve_fortran")
      implicit none
...
!>**** C interface of H-tensor solve
   ...
   !> @param ptree_Cptr: the structure containing process tree
   !> @param msh_Cptr: the structure containing points and ordering information
   subroutine C_BPACK_MD_Solve(Ndim, x, b, Nloc, Nrhs, bmat_Cptr, option_Cptr, stats_Cptr, ptree_Cptr, msh_Cptr) bind(c, name="s_c_bpack_md_solve")
      implicit none
      integer Ndim
...
!>**** C interface for getting the starting indices (w.r.t. msh(dim_i)%idxs) of each local middle-level blocks
   ...
   !> @param ptree_Cptr: the structure containing process tree
   !> @param msh_Cptr: the structure containing points and ordering information
   subroutine C_BPACK_MD_Get_Local_Midlevel_Blocks(Ndim, nc_m, head_array,bmat_Cptr, option_Cptr, ptree_Cptr, msh_Cptr) bind(c, name="s_c_bpack_md_get_local_midlevel_blocks")
      implicit none
      integer Ndim
...
!>**** C interface for getting matrix diagonal blocks at an arbitrary tree level
   ...
   subroutine C_BPACK_Get_Anylevel_Blocks(level, nblock, block_info, mesh_range, bmat_Cptr, option_Cptr, ptree_Cptr, msh_Cptr) bind(c, name="s_c_bpack_get_anylevel_blocks")
      implicit none
...
!>**** C interface for getting tensor-product diagonal blocks at an arbitrary tree level
   ...
   subroutine C_BPACK_MD_Get_Anylevel_Blocks(Ndim, level, nblock, block_info, mesh_range, bmat_Cptr, option_Cptr, ptree_Cptr, msh_Cptr) bind(c, name="s_c_bpack_md_get_anylevel_blocks")
      implicit none
      integer Ndim
...
!>**** C interface of a blackbox iterative solver with separate operator and preconditioner callbacks
   ...
   subroutine C_BPACK_Iter_Usermatvec_Precon(x, b, Nloc, Nrhs, option_Cptr, stats_Cptr, ptree_Cptr, ker_Cptr, C_FuncHMatVec, C_FuncHPreconMatVec, C_QuantApp) bind(c, name="s_c_bpack_iter_usermatvec_precon")
      implicit none
...
!>**** C interface of a blackbox tensor iterative solver with separate operator and preconditioner callbacks
   ...
   subroutine C_BPACK_MD_Iter_Usermatvec_Precon(Ndim, x, b, Nloc, Nrhs, option_Cptr, stats_Cptr, ptree_Cptr, ker_Cptr, C_FuncHMatVec_MD, C_FuncHPreconMatVec_MD, C_QuantApp) bind(c, name="s_c_bpack_md_iter_usermatvec_precon")
      implicit none
      integer Ndim
      integer Nloc(Ndim), Nrhs
      DT::x(product(Nloc), Nrhs), b(product(Nloc), Nrhs)
...
!>**** C interface of butterfly-vector multiplication
   ...
   !> @param ptree_Cptr: the structure containing process tree
   !> @param trans: 'N', 'C' or 'T'
   subroutine C_BF_Mult(trans, xin, xout, Ninloc, Noutloc, Ncol, bf_for_Cptr, option_Cptr, stats_Cptr, ptree_Cptr) bind(c, name="s_c_bf_mult")
      implicit none
      real(kind=8) t1, t2
...
!>**** C interface of parallel extraction of a list of intersections from a block
   ...
   subroutine C_BF_ExtractElement(block_Cptr, option_Cptr, msh_Cptr, stats_Cptr, ptree_Cptr, Ninter, Nallrows, Nallcols, Nalldat_loc, allrows, allcols, alldat_loc, rowidx, colidx, pgidx, Npmap, pmaps) bind(c, name="s_c_bf_extractelement")
      implicit none
...
!>**** C interface of parallel extraction of a list of intersections from a Bmat matrix
   ...
   !> @param Nallcols: total number of columns
   !> @param Nalldat_loc: total number of local entries
   subroutine C_BPACK_ExtractElement(bmat_Cptr, option_Cptr, msh_Cptr, stats_Cptr, ptree_Cptr, Ninter, Nallrows, Nallcols, Nalldat_loc, allrows, allcols, alldat_loc, rowidx, colidx, pgidx, Npmap, pmaps) bind(c, name="s_c_bpack_extractelement")
      implicit none
      type(c_ptr), intent(in) :: bmat_Cptr
      type(c_ptr), intent(in) :: ptree_Cptr
      type(c_ptr), intent(in) :: option_Cptr
...
!>**** C interface of BPACK-vector multiplication
   ...
   !> @param ptree_Cptr: the structure containing process tree
   !> @param trans: 'N', 'C' or 'T'
   subroutine C_BPACK_Mult(trans, xin, xout, Ninloc, Noutloc, Ncol, bmat_Cptr, option_Cptr, stats_Cptr, ptree_Cptr) bind(c, name="s_c_bpack_mult_fortran")
      implicit none
      real(kind=8) t1, t2
      integer Ninloc, Noutloc, Ncol
      DT::xin(Ninloc, Ncol), xout(Noutloc, Ncol)
...
!>**** C interface of BPACK_MD_Mult for BPACK_MD-vector multiply
   ...
   !> @param msh_Cptr: the structure containing points and ordering information
   !> @param trans: 'N', 'C' or 'T'
   subroutine C_BPACK_MD_Mult(Ndim,trans, xin, xout, Ninloc, Noutloc, Ncol, bmat_Cptr, option_Cptr, stats_Cptr, ptree_Cptr, msh_Cptr) bind(c, name="s_c_bpack_md_mult")
      implicit none
      real(kind=8) t1, t2
      integer Ndim
...
!>**** C interface of BPACK(inverse)-vector multiplication
   ...
   subroutine C_BPACK_Inv_Mult(trans, xin, xout, Ninloc, Noutloc, Ncol, bmat_Cptr, option_Cptr, stats_Cptr, ptree_Cptr) bind(c, name="s_c_bpack_inv_mult")
      implicit none
      real(kind=8) t1, t2
      integer Ninloc, Noutloc, Ncol
      DT::xin(Ninloc, Ncol), xout(Noutloc, Ncol)
...
!>**** C interface of deleting statistics
   !> @param stats_Cptr: the structure containing statistics
   subroutine C_BPACK_Deletestats(stats_Cptr) bind(c, name="s_c_bpack_deletestats")
      implicit none
      type(c_ptr), intent(inout) :: stats_Cptr
      type(Hstat), pointer::stats
...
!>**** C interface of deleting process tree
   !> @param ptree_Cptr: the structure containing process tree
   subroutine C_BPACK_Deleteproctree(ptree_Cptr) bind(c, name="s_c_bpack_deleteproctree")
      implicit none
...
!>**** C interface of deleting mesh
   ...
   subroutine C_BPACK_Deletemesh(msh_Cptr) bind(c, name="s_c_bpack_deletemesh")
      implicit none
...
!>**** C interface of deleting mesh
   !> @param Ndim: dimensionality of the kernel
   !> @param msh_Cptr: the structure containing mesh
   subroutine C_BPACK_MD_Deletemesh(Ndim, msh_Cptr) bind(c, name="s_c_bpack_md_deletemesh")
      implicit none
      type(c_ptr), intent(inout) :: msh_Cptr
      type(mesh), pointer::msh(:)
      integer :: Ndim, dim_i
...
!>**** C interface of deleting kernelquant
   ...
   subroutine C_BPACK_Deletekernelquant(ker_Cptr) bind(c, name="s_c_bpack_deletekernelquant")
      implicit none
...
!>**** C interface of deleting HOBF
   !> @param bmat_Cptr: the structure containing the hierarchical matrix
   subroutine C_BPACK_Delete(bmat_Cptr) bind(c, name="s_c_bpack_delete")
      implicit none
      type(c_ptr), intent(inout) :: bmat_Cptr
      type(Bmatrix), pointer::bmat
...
!>**** C interface of deleting a BF
   ...
   subroutine C_BF_DeleteBF(bf_Cptr) bind(c, name="s_c_bf_deletebf")
      implicit none
...
!>**** C interface of deleting Hoption
   ...
   subroutine C_BPACK_Deleteoption(option_Cptr) bind(c, name="s_c_bpack_deleteoption")
      implicit none
...
!> @param v_bugfix: bugfix version number
   subroutine C_BPACK_GetVersionNumber(v_major, v_minor, v_bugfix) bind(c, name="s_c_bpack_getversionnumber")
      implicit none
...
!> @param idx_child: node number in one of the two child trees
   subroutine C_BPACK_TreeIndex_Merged2Child(idx_merge, idx_child) bind(c, name="s_c_bpack_treeindex_merged2child")
      implicit none
...
!>**** C interface of BPACK log-determinant
   ...
   !> @param bmat_Cptr: the structure containing BPACK
   !> @param option_Cptr: the structure containing option
   subroutine C_BPACK_Logdet(phase, logabsdet, option_Cptr, bmat_Cptr) bind(c, name="s_c_bpack_logdet_fortran")
      implicit none
      DTR:: logabsdet
      DT:: phase
      type(c_ptr), intent(in) :: bmat_Cptr
