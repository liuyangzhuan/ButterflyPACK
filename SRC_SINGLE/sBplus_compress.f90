...
module s_Bplus_compress
   use s_BPACK_DEFS
   use s_MISC_Utilities
   use s_Bplus_randomizedop
   ...
   subroutine BF_MD_compress_N_oneblock_R_sample(Ndim, dim_MD, subtensors, blocks, boundary_map, Nboundall,Ninadmissible, groupm_start, option, stats, msh, ker, ptree, index_ij, bb_m, level, flops)
      ...
      do dim_i=1,Ndim*2
         if(option%fastsample_tensor==2 .and. dim_i>Ndim .and. dim_i/=Ndim+dim)then
            ...
            sampleidx(dim_i) = min(max(ceiling_safe(option%sample_para*nn_scalar*overrate),Nsample_min_MD), min(mmnn(dim_i),option%rmax))
            if (level == 0) sampleidx(dim_i) = min(max(ceiling_safe(option%sample_para_outer*nn_scalar*overrate),Nsample_min_MD), min(mmnn(dim_i),option%rmax))
            allocate(select_idx(dim_i)%dat(sampleidx(dim_i)))
   ...
   subroutine BF_MD_compress_N_oneblock_C_sample(Ndim, dim_MD, subtensors, blocks, boundary_map, Nboundall,Ninadmissible, groupm_start, option, stats, msh, ker, ptree, index_ij, bb_m, level, flops)
      ...
      do dim_i=1,Ndim*2
         ...
         if(option%fastsample_tensor==2 .and. dim_i<=Ndim .and. dim_i/=dim)then
            ...
            sampleidx(dim_i) = min(max(ceiling_safe(option%sample_para*mm_scalar*overrate),Nsample_min_MD), min(mmnn(dim_i),option%rmax))
            if (level == level_butterfly+1) sampleidx(dim_i) = min(max(ceiling_safe(option%sample_para_outer*mm_scalar*overrate),Nsample_min_MD), min(mmnn(dim_i),option%rmax))
            allocate(select_idx(dim_i)%dat(sampleidx(dim_i)))
            call linspaceI(1, mmnn(dim_i), sampleidx(dim_i), select_idx(dim_i)%dat(1:sampleidx(dim_i)))
         endif
      enddo
