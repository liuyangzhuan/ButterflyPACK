...
def c_py_bpack_setup(sp):
    # Define the function signatures as shown in your original code

    sp.c_py_bpack_init_compute.restype = None
    sp.c_py_bpack_init_compute.argtypes = [
    ...
    sp.c_py_bpack_factor.restype = None
    sp.c_py_bpack_factor.argtypes = [ctypes.POINTER(ctypes.c_void_p)]
    sp.c_py_bpack_solve.restype = None
    sp.c_py_bpack_solve.argtypes = [ctypes.POINTER(ctypes.c_void_p), ctypes.c_int, ctypes.POINTER(ctypes.c_float*2)]
    sp.c_py_bpack_mult.restype = None
    sp.c_py_bpack_mult.argtypes = [ctypes.POINTER(ctypes.c_void_p), ctypes.c_int, ctypes.c_char_p, ctypes.POINTER(ctypes.c_float*2)]
    sp.c_py_bpack_logdet.restype = None
    sp.c_py_bpack_logdet.argtypes = [ctypes.POINTER(ctypes.c_void_p), ctypes.POINTER(ctypes.c_float*2), ctypes.POINTER(ctypes.c_float)]
    sp.c_py_bpack_free.restype = None
    sp.c_py_bpack_free.argtypes = [ctypes.POINTER(ctypes.c_void_p)]
    sp.c_py_bpack_terminate.restype = None
    sp.c_py_bpack_terminate.argtypes = None
...
def c_py_bpack_load():
    ...
    DLL = os.path.abspath(__file__ + "/../../") + '/cbutterflypack_python' + pos
    if os.path.exists(DLL):
        DLLFOUND = True
    elif INSTALLDIR is not None:
        DLL = os.path.join(INSTALLDIR, 'cbutterflypack_python' + pos)
        if os.path.exists(DLL):
            DLLFOUND = True
    else:
        DLL = os.path.join('./cbutterflypack_python' + pos)
        if os.path.exists(DLL):
            DLLFOUND = True
    if DLLFOUND:
        ...
        raise Exception("Cannot find the cbutterflypack_python library. Try to set the BPACK_PYTHON_LIB_PATH environment variable correctly.")
...
def bpack_factor(payload, verbosity=False, nofactor=False, fid=0):
    ...
    if verbosity==True:
        print(f"ID {fid}: Time spent in c_py_bpack_init_compute: {end - start} seconds")
    ...
    if nofactor==False:
        ...
        if verbosity==True:
            print(f"ID {fid}: Time spent in c_py_bpack_factor: {end - start} seconds")
...
def bpack_solve(vec, verbosity=False,fid=0):
    ...
    if verbosity==True:
        print(f"ID {fid}: Time spent in c_py_bpack_solve: {end - start} seconds")
...
def bpack_mult(vec, trans, verbosity=False,fid=0):
    ...
    if verbosity==True:
        print(f"ID {fid}: Time spent in c_py_bpack_mult: {end - start} seconds")
    vec_out = vec_out.reshape(orig_shape)
    return vec_out
...
def bpack_logdet(verbosity=False,fid=0):
    ...
    if verbosity==True:
        print(f"ID {fid}: Time spent in c_py_bpack_logdet: {end - start} seconds")
    return sign,log_det
...
def bpack_free(verbosity=False,fid=0):
    ...
    if verbosity==True:
        print(f"ID {fid}: Time spent in c_py_bpack_free: {end - start} seconds")
...
def bpack_terminate(verbosity=False):
    ...
    if verbosity==True:
        print(f"ID {0}: Time spent in c_py_bpack_terminate: {end - start} seconds")
