...
while True:
    ...
    if(flag=="init"):
        ...
        sp = cPy_BPACK_wrapper.c_py_bpack_load()
        ####################### initialization
        pyobjs[fid] = ctypes.c_void_p()
        maxrank = ctypes.c_int(0)
        sp.c_py_bpack_init_compute(
        ...
        if(rank==0):
            print("maxrank from c_py_bpack_init_compute:", maxrank.value)

    elif(flag=="factor"):
        ####################### factor
        sp.c_py_bpack_factor(
            ctypes.byref(pyobjs[fid]),            # void **pyobj
        )
    elif(flag=="solve"):
        ####################### solve
        ...
        nrhs=-1
        xb = np.random.rand(1).astype(np.complex64)
        if rank == 0:
            with open(f"{DATA_FILE}.{fid}", "rb") as f:
                xb,nrhs = pickle.load(f)
            xb = np.ascontiguousarray(xb, dtype=np.complex64)
        nrhs = bcast(nrhs, root=0)
        sp.c_py_bpack_solve(
            ctypes.byref(pyobjs[fid]),            # void **pyobj
            nrhs,                           # int nrhs
            xb.ctypes.data_as(ctypes.POINTER(ctypes.c_float*2)),
        )
        if rank == 0:
        ...
        ####################### log-determinant
        sign = ctypes.c_float*2(1)
        logdet = ctypes.c_float(0.0)
        sp.c_py_bpack_logdet(
            ctypes.byref(pyobjs[fid]),            # void **pyobj
        ...
        ####################### free stuff
        sp.c_py_bpack_free(ctypes.byref(pyobjs[fid]))
    elif(flag=="terminate"):
        sp.c_py_bpack_terminate()
        break
