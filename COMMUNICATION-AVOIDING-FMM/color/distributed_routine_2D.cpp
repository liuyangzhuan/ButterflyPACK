...
DataType verify_solution_fft(
    ...
    // Create FFT kernel
    kernel::LaplaceKernel2D_FFT<DataType> fft_kernel(
        grid_points.data(),
        grid_size,
        N
    );
...
void hierarchical_factorization_parallel(
    ...
    for (int current_level = leaf_level; current_level >= 1; current_level--) {
        ...
        if (current_level > 1) {
            ...
            color_bins.shrink_to_fit(); // optional
            ...
            for (int64_t local_idx = 0; local_idx < static_cast<int64_t>(level.local_boxes.size()); ++local_idx) {
                ...
                const int color_id = static_cast<int>(morton_idx & (num_colors - 1)); // == morton_idx % num_colors
            ...
            for (int counter = 0; counter < num_colors; ++counter) {
                ...
                for (int64_t morton_idx : color_list) {
                    ...
                    compute_and_modify(dimension,
                        &box, level, kernel,
                        scratch,
                        tolerance, is_symmetric, is_hermitian, &pending_updates,
                ...
                auto assist_comm_duration_raw = std::chrono::high_resolution_clock::duration{};
                
                
                // verify_modified_interaction_symmetry(tree->levels[current_level], 1e-16, true);
                if(counter == num_colors - 1) {
                    assist_comm_duration_raw = transport_and_apply_factor_updates_symmetric_onehop(
                        tree, current_level, kernel, pending_updates, false);
                    update_neighbor_slicing_for_level(level, is_symmetric);
                }

                auto assist_duration = std::chrono::duration_cast<std::chrono::milliseconds>(assist_comm_duration_raw);
                if (verbose && rank == 0) {
...
void hierarchical_solve_parallel(
    ...
    for (int level = leaf_level; level >= 1; level--) {
        ...
        if (level >= 2) {
            ...
            std::vector<std::vector<int64_t>> color_bins((size_t)num_colors);
            for (int64_t morton = tree_level.local_morton_start;
                morton <= tree_level.local_morton_end; ++morton)
            {
                const int color_id = (int)(morton & (num_colors - 1)); // == morton % num_colors
                color_bins[(size_t)color_id].push_back(morton);
            }
            ...
            for (int color_id = 0; color_id < num_colors; ++color_id) {
                ...
                clear_pending_solve_updates_memory(pending_solve);
            }
    ...
    for (int level = 1; level <= leaf_level; level++) {
        ...
        if (level >= 2) {
           ...
           // 2^d color-wave backward sweep (reverse order vs forward):
            ...
            // Bin LOCAL mortons by color = morton % num_colors (power-of-2 => bitmask)
            std::vector<std::vector<int64_t>> color_bins((size_t)num_colors);
            for (int64_t morton = tree_level.local_morton_start;
                morton <= tree_level.local_morton_end; ++morton)
            {
                const int color_id = (int)(morton & (num_colors - 1));
                color_bins[(size_t)color_id].push_back(morton);
            }

            // Backward: reverse color order, and reverse within-color order
            for (int color_id = num_colors - 1; color_id >= 0; --color_id) {
...
int main(int argc, char* argv[]) {
    ...
    try {
        if (rank == 0) {
            std::cout << "=== Hierarchical Factorization Test (2D Laplace Kernel) ===" << std::endl;
        }
        ...
        kernel::LaplaceKernel2D<double> kernel(N);
        
        fmm::HierarchicalFactorization<double, double, kernel::LaplaceKernel2D<double>> 
            factorizer(N, fmm::MatrixProperty::SYMMETRIC, &kernel, 2, 
                      fmm::FactorizationMethod::CHOLESKY, 32);
