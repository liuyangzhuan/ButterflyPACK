...
struct d_c_bpack_QuantApp{
    py::object py_func;
    py::object py_meta;
};

struct d_bpack_handle{
    F2Cptr bmat;
    F2Cptr option;
    F2Cptr stats;
    F2Cptr msh;
...
inline void d_c_bpack_FuncZmn(int *m, int *n, double *val, C2Fptr quant) {
}
...
inline void d_c_bpack_FuncDistmn(int *m, int *n, double *val, C2Fptr quant) {
}

// The compressibility function wrapper required by the Fortran ButterflyPACK interface
// not needed in this driver, but need to provide a dummy function to the interface
inline void d_c_bpack_FuncNearFar(int *m, int *n, int *val, C2Fptr quant) {
}

// The blocked sampling function wrapper required by the Fortran ButterflyPACK interface
inline void d_c_bpack_FuncZmnBlock(int* Ninter, int* Nallrows, int* Nallcols, int64_t* Nalldat_loc, int* allrows, int* allcols, double* alldat_loc, int* rowidx,int* colidx, int* pgidx, int* Npmap, int* pmaps, C2Fptr quant) {
    d_c_bpack_QuantApp* Q = (d_c_bpack_QuantApp*) quant;
    ...
    for (int nn=0;nn<*Ninter;nn++){
      ...
      if(nprow*npcol==1){
        ...
        std::cout<<"nprow*npcol>1 in d_c_bpack_FuncZmnBlock"<<std::endl;
        exit(0);
      }
    }
    ...
    for (int nn1=0;nn1<NinterNew;nn1++){
      ...
      for (int idxc=0;idxc<nc;idxc++){
		...
		py::array_t<double, py::array::f_style | py::array::forcecast> arr(result);
		...
		std::memcpy(&(alldat_loc[idx_val_map[nn1]]), src, sizeof(double) * nr * nc);
...
void d_py_bpack_logdet(void ** pyobj, double * phase, double * logabsdet)
{
    d_bpack_handle* bpack_obj = (d_bpack_handle*)(*pyobj);
    d_c_bpack_logdet(phase, logabsdet,&(bpack_obj->option),&(bpack_obj->bmat));
    *pyobj = (void*)bpack_obj;
}


void d_py_bpack_init_compute(int Npo, int Ndim, double* dat_ptr, void ** pyobj, int* rankmax, int argc, char* argv[])
{
	pybind11::gil_scoped_acquire gil;
    d_bpack_handle* bpack_obj = (d_bpack_handle*)malloc(sizeof(d_bpack_handle));
	// bpack_obj->quant_ptr = (d_c_bpack_QuantApp*)malloc(sizeof(d_c_bpack_QuantApp));
	bpack_obj->quant_ptr = new d_c_bpack_QuantApp();
	...
	d_c_bpack_QuantApp *quant_ptr=bpack_obj->quant_ptr;
	...
	d_c_bpack_createptree(&size, groups, &Fcomm, &(bpack_obj->ptree));
	d_c_bpack_createoption(&(bpack_obj->option));
	d_c_bpack_createstats(&(bpack_obj->stats));


	// set default butterflypack options
	d_c_bpack_set_D_option(&(bpack_obj->option), "tol_comp", tol);
	d_c_bpack_set_D_option(&(bpack_obj->option), "tol_rand", tol);
	d_c_bpack_set_D_option(&(bpack_obj->option), "tol_Rdetect", tol*1e-1);
	d_c_bpack_set_D_option(&(bpack_obj->option), "sample_para", sample_para);
	d_c_bpack_set_D_option(&(bpack_obj->option), "near_para", near_para);
	d_c_bpack_set_I_option(&(bpack_obj->option), "nogeo", nogeo);
	d_c_bpack_set_I_option(&(bpack_obj->option), "Nmin_leaf", Nmin);
	d_c_bpack_set_I_option(&(bpack_obj->option), "RecLR_leaf", com_opt);
	d_c_bpack_set_I_option(&(bpack_obj->option), "xyzsort", sort_opt);
	d_c_bpack_set_I_option(&(bpack_obj->option), "ErrFillFull", checkerr);
	d_c_bpack_set_I_option(&(bpack_obj->option), "BACA_Batch", batch);
	d_c_bpack_set_I_option(&(bpack_obj->option), "LR_BLK_NUM", bnum);
	d_c_bpack_set_I_option(&(bpack_obj->option), "cpp", cpp);
	d_c_bpack_set_I_option(&(bpack_obj->option), "LRlevel", lrlevel);
	d_c_bpack_set_I_option(&(bpack_obj->option), "knn", knn);
	d_c_bpack_set_I_option(&(bpack_obj->option), "verbosity", 1);
	d_c_bpack_set_I_option(&(bpack_obj->option), "less_adapt", 1);
	d_c_bpack_set_I_option(&(bpack_obj->option), "itermax", 10);
	d_c_bpack_set_I_option(&(bpack_obj->option), "ErrSol", 0);
	d_c_bpack_set_I_option(&(bpack_obj->option), "format", format);
	d_c_bpack_set_I_option(&(bpack_obj->option), "elem_extract", elem_extract);

	// set command-line butterflypack options
	d_c_bpack_set_option_from_command_line(c_argc, argv_ptr,bpack_obj->option);

	// print out butterflypack options
	d_c_bpack_printoption(&(bpack_obj->option),&(bpack_obj->ptree));

    // construct matrix with geometrical points
	d_c_bpack_construct_init(&Npo, &Ndim, dat_ptr, nns_ptr,&nlevel, tree, perms, &myseg, &(bpack_obj->bmat), &(bpack_obj->option), &(bpack_obj->stats), &(bpack_obj->msh), &(bpack_obj->ker), &(bpack_obj->ptree), &d_c_bpack_FuncDistmn, &d_c_bpack_FuncNearFar, quant_ptr);
	d_c_bpack_construct_element_compute(&(bpack_obj->bmat), &(bpack_obj->option), &(bpack_obj->stats), &(bpack_obj->msh), &(bpack_obj->ker), &(bpack_obj->ptree), &d_c_bpack_FuncZmn, &d_c_bpack_FuncZmnBlock, quant_ptr);

    double vtmp;
    d_c_bpack_getstats(&(bpack_obj->stats),"Rank_max",&vtmp);
    *rankmax=int(vtmp);
...
void d_py_bpack_factor(void ** pyobj)
{
	d_bpack_handle* bpack_obj = (d_bpack_handle*)(*pyobj);
	d_c_bpack_factor(&(bpack_obj->bmat),&(bpack_obj->option),&(bpack_obj->stats),&(bpack_obj->ptree),&(bpack_obj->msh));
	*pyobj = (void*)bpack_obj;
}



void d_py_bpack_solve(void ** pyobj, int nrhs, double   *xb_global)
{
	d_bpack_handle* bpack_obj = (d_bpack_handle*)(*pyobj);
	int idxs, m_loc, fst_row, m, m_loc1, fst_row1;

	d_c_bpack_localindices(&(bpack_obj->msh), &idxs, &m_loc, &m);

    /* Get the local B and X*/
    double* b= (double*)malloc(m_loc * nrhs*sizeof(double));
    double* x= (double*)malloc(m_loc * nrhs*sizeof(double));

	d_c_bpack_vector_global2local(&(bpack_obj->ptree), &(bpack_obj->msh), &nrhs, xb_global, b);
	d_c_bpack_solve(x,b,&m_loc,&nrhs,&(bpack_obj->bmat),&(bpack_obj->option),&(bpack_obj->stats),&(bpack_obj->ptree));
	d_c_bpack_vector_local2global(&(bpack_obj->ptree), &(bpack_obj->msh), &nrhs, x, xb_global);
...
void d_py_bpack_mult(void ** pyobj, int nrhs, char const *trans, double   *xy_global)
{
	d_bpack_handle* bpack_obj = (d_bpack_handle*)(*pyobj);
	int idxs, m_loc, fst_row, m, m_loc1, fst_row1;
    ...
    double* y= (double*)malloc(m_loc * nrhs*sizeof(double));
    double* x= (double*)malloc(m_loc * nrhs*sizeof(double));

	d_c_bpack_vector_global2local(&(bpack_obj->ptree), &(bpack_obj->msh), &nrhs, xy_global, x);
	d_c_bpack_mult(trans, x, y, &m_loc, &m_loc, &nrhs, &(bpack_obj->bmat),&(bpack_obj->option),&(bpack_obj->stats),&(bpack_obj->ptree));
	d_c_bpack_vector_local2global(&(bpack_obj->ptree), &(bpack_obj->msh), &nrhs, y, xy_global);
...
void d_py_bpack_free(void ** pyobj)
{
	py::gil_scoped_acquire gil;
	d_bpack_handle* bpack_obj = (d_bpack_handle*)(*pyobj);

	d_c_bpack_deletestats(&(bpack_obj->stats));
	d_c_bpack_deleteproctree(&(bpack_obj->ptree));
	d_c_bpack_deletemesh(&(bpack_obj->msh));
	d_c_bpack_deletekernelquant(&(bpack_obj->ker));
	d_c_bpack_delete(&(bpack_obj->bmat));
	d_c_bpack_deleteoption(&(bpack_obj->option));
