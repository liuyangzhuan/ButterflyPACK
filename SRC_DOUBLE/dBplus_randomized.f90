...
module d_Bplus_randomized
! use Utilites_randomized

use d_misc
use d_HODLR_Utilities
	...
	type is (blockplus)
		...
		! type is (hobf)	
			...
			if(option%TwoLayerOnly==1)then
				...
				! bb = boxindex(1)
				! blocks => d_Bplus_randomized_constr(1)%LL(ll)%matrices_block(bb)
...
subroutine d_Bplus_randomized_constr(level_butterfly,bplus_o,operand,rank0_inner,rankrate_inner,blackbox_MVP_dat_inner,rank0_outter,rankrate_outter,blackbox_MVP_dat_outter,error_inout,strings,option,stats,ptree)
   ...
   implicit none
	...
	type(Hoption)::option
	type(Hstat)::stats
	type(blockplus) :: d_Bplus_randomized
	procedure(BF_MVP_blk)::blackbox_MVP_dat_inner,blackbox_MVP_dat_outter
	type(proctree)::ptree
	error_inout=0
	! Memory = 0	
	call assert(bplus_o%Lplus>=2,'this is not a multi Bplus in d_Bplus_randomized')
	
	call Bplus_Init_FromInput(bplus_o,d_Bplus_randomized)
	
	! write(*,*)'hhhhh1',d_Bplus_randomized%LL(1)%matrices_block(1)%level
	n1 = OMP_get_wtime()

	rankthusfar = 0	
	do bb =1,d_Bplus_randomized%LL(2)%Nbound
		! ! write(*,*)bb,d_Bplus_randomized%LL(2)%Nbound,'dddd'

		
		! rank0_inner = ho_bf1%levels(level_c)%BP(2*rowblock-1)%LL(2)%rankmax
		! rankrate_inner = 2.0d0
		block_o => d_Bplus_randomized%LL(2)%matrices_block(bb)
		! ho_bf1%ind_lv = level_c
		! ho_bf1%ind_bk = rowblock
		
		call Bplus_MultiLrandomized_Onesubblock(rank0_inner,rankrate_inner,rankthusfar,block_o,operand,blackbox_MVP_dat_inner,error,strings,option,stats,ptree,d_Bplus_randomized)		
	...
	! rate_outter=1.2d0
	call BF_randomized(level_butterfly,rank0_outter,rankrate_outter,d_Bplus_randomized%LL(1)%matrices_block(1),operand,blackbox_MVP_dat_outter,error,'Outter',option,stats,ptree,d_Bplus_randomized)
	error_inout = max(error_inout, error)

	
	call delete_Bplus(bplus_o)
	call copy_delete_Bplus(d_Bplus_randomized,bplus_o,Memory)
	! deallocate(d_Bplus_randomized)
...
end subroutine d_Bplus_randomized_constr
...
subroutine Bplus_Init_FromInput(Bplus,d_Bplus_randomized)
	use d_misc
    use d_HODLR_DEFS
	...
	type(blockplus)::Bplus,d_Bplus_randomized
	integer seed_myid(2)
	integer time(8)
	integer ll,bb
	...
	! allocate(d_Bplus_randomized)
	
	d_Bplus_randomized%level = Bplus%level
	d_Bplus_randomized%col_group = Bplus%col_group
	d_Bplus_randomized%row_group = Bplus%row_group
	d_Bplus_randomized%Lplus = Bplus%Lplus
	d_Bplus_randomized%boundary = Bplus%boundary
	
	allocate(d_Bplus_randomized%LL(LplusMax))
	

	! d_Bplus_randomized%LL(1)%Nbound = 1
	! allocate(d_Bplus_randomized%LL(1)%matrices_block(1))
	! d_Bplus_randomized%LL(1)%matrices_block(1)%level = d_Bplus_randomized%level
	! d_Bplus_randomized%LL(1)%matrices_block(1)%col_group = d_Bplus_randomized%col_group
	! d_Bplus_randomized%LL(1)%matrices_block(1)%row_group = d_Bplus_randomized%row_group			
	! d_Bplus_randomized%LL(1)%matrices_block(1)%style = Bplus%LL(1)%matrices_block(1)%style
	allocate(d_Bplus_randomized%LL(1)%boundary_map(1))
	d_Bplus_randomized%LL(1)%boundary_map(1) = Bplus%LL(1)%boundary_map(1)
	
	do ll=1,LplusMax
	d_Bplus_randomized%LL(ll)%Nbound = 0
	end do	
	
	do ll=1,LplusMax-1
		if(Bplus%LL(ll)%Nbound>0)then

			d_Bplus_randomized%LL(ll)%rankmax = Bplus%LL(ll)%rankmax
			d_Bplus_randomized%LL(ll)%Nbound = Bplus%LL(ll)%Nbound
			allocate(d_Bplus_randomized%LL(ll)%matrices_block(d_Bplus_randomized%LL(ll)%Nbound))
			
			do bb =1,d_Bplus_randomized%LL(ll)%Nbound
				d_Bplus_randomized%LL(ll)%matrices_block(bb)%row_group = Bplus%LL(ll)%matrices_block(bb)%row_group
				d_Bplus_randomized%LL(ll)%matrices_block(bb)%col_group = Bplus%LL(ll)%matrices_block(bb)%col_group
				d_Bplus_randomized%LL(ll)%matrices_block(bb)%style = Bplus%LL(ll)%matrices_block(bb)%style
				d_Bplus_randomized%LL(ll)%matrices_block(bb)%level = Bplus%LL(ll)%matrices_block(bb)%level
			end do
			
			
			if(Bplus%LL(ll+1)%Nbound==0)then		
				d_Bplus_randomized%LL(ll+1)%Nbound=0
			else 
				...
				allocate(d_Bplus_randomized%LL(ll+1)%boundary_map(Nboundall))
				! write(*,*)shape(Bplus%LL(ll+1)%boundary_map),shape(d_Bplus_randomized%LL(ll+1)%boundary_map),'didi',ll
				
				! write(*,*)'gali',ll,Nboundall,shape(Bplus%LL(ll+1)%boundary_map)
				d_Bplus_randomized%LL(ll+1)%boundary_map = Bplus%LL(ll+1)%boundary_map
			end if
		else 
			exit
		end if
