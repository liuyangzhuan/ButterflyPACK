      SUBROUTINE pcgetf2mod( M, N, A, IA, JA, DESCA, THRESH, IPIV, INFO )
*
...
*  =======
*
*  pcgetf2mod computes an LU factorization of a general M-by-N
...
*     .. External Subroutines ..
     ...
     $                   igebs2d, infog2l, pcamax, pcgeru,
     $                   pcscal, pcswap, pb_topget, pxerbla, pcelset
*     ..
*     .. Intrinsic Functions ..
      INTRINSIC          min, mod
*     ..
...
*
      IF( info.NE.0 ) THEN
         CALL pxerbla( ictxt, 'pcgetf2mod', -info )
...
*
            CALL pcamax( m-j+ja, gmax, ipiv( iia+j-ja ), a, i, j,
     $                   desca, 1 )
...
*
               IF( j-ja+1.LT.m )
     $            CALL pcscal( m-j+ja-1, one / gmax, a, i+1, j,
     $                         desca, 1 )
...
*
            IF( j-ja+1.LT.mn ) THEN
               CALL pcgeru( m-j+ja-1, n-j+ja-1, -one, a, i+1, j, desca,
