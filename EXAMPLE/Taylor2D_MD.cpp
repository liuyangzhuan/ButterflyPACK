...
if(myrank==master_rank){
	...
	for(int ii=1;ii<=Npo;ii++){
		data_geo[(ii-1) * Ndim] =ii*dl;
		data_geo[(ii-1) * Ndim+1] =ii*dl;
	}
	...
	int* myseg=new int[Ndim];     // local number of unknowns
	int* perms = new int[Npo*Ndim]; //permutation vector returned by HODLR
	int* groups = new int[size];
	int* Npos = new int[Ndim];
	...
	z_c_bpack_md_construct_init(Npos, &Ndim, dat_ptr, nns_ptr, perms, myseg, &bmat, &option, &stats, &msh, &kerquant, &ptree, &C_FuncDistmn, &C_FuncNearFar, quant_ptr);
