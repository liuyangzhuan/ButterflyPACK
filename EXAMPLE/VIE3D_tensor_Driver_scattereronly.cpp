...
if(myrank==master_rank){
  ...
  while ((c = getopt_long_only
    ...
    case 39: {
      std::istringstream iss(optarg);
      iss >> bdiag_tol_rand;
      bdiag_tol_Rdetect = bdiag_tol_rand*0.1;
    } break;
    case 40: {
      std::istringstream iss(optarg);
  ...
  if(bdiag_precon==1){
    ...
    z_c_bpack_set_I_option(&(quant_ptr_bf_s2s->option_diag), "elem_extract", 0); // entry extraction is not needed for tree discovery
