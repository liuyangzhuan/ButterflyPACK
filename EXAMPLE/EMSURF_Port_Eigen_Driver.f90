...
!> @file
!> @brief This is an example that uses PARPACK and ButterflyPACK to find the smallest eigen value or do characteristic mode analysis for a 3D IE system for a cavity with ports in electromagnetics.
...
# 124 "EMSURF_Port_Eigen_Driver.f90"
   ...
   !***********************************************************************
	...
	if(quant%postprocess==1 .and. maxval(order)>0)then
		...
		E_normal=0
		call z_BPACK_Mult('N', Nunk_loc, quant%nev, eigvec, E_normal, bmat_post, ptree_post, option_post, stats_post)
		do nn=1,quant%nev
			if(order(nn)>0)then
		...
		call z_delete_proctree(ptree_post)
		call z_delete_Hstat(stats_post)
		call z_delete_mesh(msh_post)
		call z_delete_kernelquant(ker_post)
		call z_BPACK_delete(bmat_post)
	endif
