...
if(allocated(quant%edge_of_patch))deallocate(quant%edge_of_patch)
...
subroutine Zelem_EMSURF_block(Ninter, allrows, allcols, alldat_loc, rowidx, colidx, pgidx, Npmap, pmaps, quant) ! interface of user-defined element extraction routine in Fortran. allrows,allcols represents indices in natural order
	...
	complex(kind=4),target::alldat_loc(:)
...
subroutine Zelem_EMSURF_oneblock(nn1,inter_map,allrows, allcols, alldat_loc, rowidx, colidx, idx_row_map,idx_col_map,idx_val_map,quant)
    ...
    real(kind=4) nr_n(3)
    complex(kind=4) ctemp,ctemp1,ctemp2,aa(3),bb(1),dg(3),dg1(3),dg2(3)
    complex(kind=4) imp,imp1,imp2,imp3, consts
    real(kind=4) temp
    real(kind=4) distance
    real(kind=4) ianl,ianl1,ianl2
    real(kind=4) area
	...
	real(kind=4):: row_sign,col_sign
	integer,parameter::Nnodes_max=3
	complex(kind=4),dimension(1:Nnodes_max,1:Nnodes_max,1:3):: temp_matrix
	complex(kind=4),dimension(1:Nnodes_max,1:Nnodes_max):: final_matrix
	...
	do idxr=1,nr
		...
		enddo
	enddo
	call c_PIKSRT_INT_Multi(idxcnt_m, 3, M_elemlist(1:idxcnt_m,:))
	M_elem_tot=0
	...
	do idxc=1,nc
		...
		enddo
	enddo
	call c_PIKSRT_INT_Multi(idxcnt_n, 3, N_elemlist(1:idxcnt_n,:))
	N_elem_tot=0
