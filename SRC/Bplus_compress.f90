	...
	if(.not. (min(blocks%M,blocks%N)>leafsize .or. (associated(gd%gdc))))then ! reach bottom level, call sequential aca
		...
		if(option%RecLR_leaf==RRQR)then
		   ...
		   ! enddo		
			...
			if(option%RecLR_leaf==BACA)call BatchACA_CompressionForward(UU,VV,blocks%headm,blocks%headn,blocks%M,blocks%N,rmax,rank,option%tol_comp,option%tol_comp,option%BACA_Batch,msh,ker,stats,element_Zmn,ptree,error)	
			if(option%RecLR_leaf==BACANOVER)call BatchACA_CompressionForward_noOverlap(UU,VV,blocks%headm,blocks%headn,blocks%M,blocks%N,rmax,rank,option%tol_comp,option%tol_comp,option%BACA_Batch,msh,ker,stats,element_Zmn,ptree,error)	
...
subroutine BatchACA_CompressionForward_noOverlap(matU,matV,header_m,header_n,M,N,rmax,rank,tolerance,SVD_tolerance,bsize,msh,ker,stats,element_Zmn,ptree,error)
...
end subroutine BatchACA_CompressionForward_noOverlap
