...
subroutine Bplus_Sblock_randomized_memfree(level_c,rowblock,Memory)
	...
	if(bplus%Lplus==1)then
		...
		if(level_butterfly==0)then
			...
			rate = 1.2d0
			call Butterfly_randomized(level_butterfly,rank0,rate,block_o,ho_bf,butterfly_block_MVP_Sblock_dat,error_inout,'Sblock')
		end if
	else 
	! write(*,*)'cacadd1',level_c,rowblock
		...
		call Buplus_randomized(Bplus,ho_bf,rank0_inner,rankrate_inner,Bplus_block_MVP_Sblock_dat,rank0_outter,rankrate_outter,Bplus_block_MVP_Outter_Sblock_dat,error_inout,'Sblock')
	...
	do level = Maxlevel_for_blocks+1,level_c+1,-1
		...
		if(level==Maxlevel_for_blocks+1)then
			...
			do ii = idx_start_diag,idx_start_diag+N_diag-1
				...
				call OneL_block_MVP_inverse_dat(ho_bf,level,ii,'N',idx_end_loc-idx_start_loc+1,num_vect_sub,vec_old(idx_start_loc:idx_end_loc,1:num_vect_sub),vec_new(idx_start_loc:idx_end_loc,1:num_vect_sub))
				! ! vec_new(idx_start_loc:idx_end_loc,1:num_vect_sub) = 	vec_old(idx_start_loc:idx_end_loc,1:num_vect_sub)	
			end do		
			n2 = OMP_get_wtime()
			! time_tmp = time_tmp + n2 - n1			
