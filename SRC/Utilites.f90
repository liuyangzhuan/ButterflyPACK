...
subroutine butterfly_block_MVP(blocks,chara,vector_a,vector_b,a,b)
    
    use MODULE_FILE
    implicit none
    
    integer blocks, n, group_m, group_n, group_mm, group_nn, index_i, index_j, na, nb, index_start, num_vectors
    ...
    num_vectors=size(vectors_block(vector_a)%vector,2)
    ...
    if (chara=='N') then
        ...
        num_blocks=2**(Maxlevel_for_blocks-level_blocks)
        ...
        do i=1, num_blocks
            ...
            do ii=1, nn
                do jj=1, num_vectors
                    ButterflyVector(0)%blocks(1,i)%matrix(ii,jj)=vectors_block(vector_a)%vector(ii+k,jj)
                enddo
            enddo
            !$omp end parallel do
        enddo 
        ...
        do level=0, level_butterfly
            if (level==0) then
                ...
                do j=1, num_groupn
                    ...
                    do ii=1, rank
                        do jj=1, num_vectors
                            ctemp=0
                            do kk=1, nn
                                ctemp=ctemp+matrices_block(blocks,0)%ButterflyV(j)%matrix(kk,ii)*ButterflyVector(0)%blocks(1,j)%matrix(kk,jj)
                            enddo
                ...
                num_groupm=matrices_block(blocks,0)%ButterflyKerl(level)%num_row
                num_groupn=matrices_block(blocks,0)%ButterflyKerl(level)%num_col
                if (num_groupn/=1) then
                    ...
                    do i=1, num_groupm
                        ...
                        do j=1, num_groupn, 2
                            index_j=int((j+1)/2)
                            nn1=size(matrices_block(blocks,0)%ButterflyKerl(level)%blocks(i,j)%matrix,2)
                            nn2=size(matrices_block(blocks,0)%ButterflyKerl(level)%blocks(i,j+1)%matrix,2)
                            mm=size(matrices_block(blocks,0)%ButterflyKerl(level)%blocks(i,j)%matrix,1)
                            ...
                            do ii=1, mm
                                do jj=1, num_vectors
                                    ctemp=0
                                    do kk=1, nn1
                                        ctemp=ctemp+matrices_block(blocks,0)%ButterflyKerl(level)%blocks(i,j)%matrix(ii,kk)*ButterflyVector(level)%blocks(index_i,j)%matrix(kk,jj)
                                    enddo
                                    do kk=1, nn2
                                        ctemp=ctemp+matrices_block(blocks,0)%ButterflyKerl(level)%blocks(i,j+1)%matrix(ii,kk)*ButterflyVector(level)%blocks(index_i,j+1)%matrix(kk,jj)
                                    enddo
                    ...
                    do i=1, num_groupm
                        index_i=int((i+1)/2)
                        nn=size(matrices_block(blocks,0)%ButterflyKerl(level)%blocks(i,1)%matrix,2)
                        mm=size(matrices_block(blocks,0)%ButterflyKerl(level)%blocks(i,1)%matrix,1)
                        ...
                        do ii=1, mm
                            do jj=1, num_vectors
                                ctemp=0                            
                                do kk=1, nn
                                    ctemp=ctemp+matrices_block(blocks,0)%ButterflyKerl(level)%blocks(i,1)%matrix(ii,kk)*ButterflyVector(level)%blocks(index_i,1)%matrix(kk,jj)
                                enddo
                ...
                do i=1, num_blocks
                    ...
                    do ii=1, mm
                        do jj=1, num_vectors
                            ctemp=0
                            do kk=1, rank
                                ctemp=ctemp+matrices_block(blocks,0)%ButterflyU(i)%matrix(ii,kk)*ButterflyVector(level+1)%blocks(i,1)%matrix(kk,jj)
                            enddo
        ...
        do index_i=1, num_blocks
            index_start=index_i-1
            header_m=basis_group(groupm_start+index_start)%head
            tailer_m=basis_group(groupm_start+index_start)%tail
            mm=tailer_m-header_m+1
            k=header_m-header_mm
            !$omp parallel do default(shared) private(ii,jj)
            do ii=1, mm
                do jj=1, num_vectors
                    vectors_block(vector_b)%vector(ii+k,jj)=b*vectors_block(vector_b)%vector(ii+k,jj)+a*ButterflyVector(level_butterfly+2)%blocks(index_i,1)%matrix(ii,jj)
                enddo
            enddo
            !$omp end parallel do
        enddo      
    ...
    elseif (chara=='T') then
        ...
        do i=1, num_blocks
            ...
            do ii=1, mm
                do jj=1, num_vectors
                    ButterflyVector(0)%blocks(i,1)%matrix(ii,jj)=vectors_block(vector_a)%vector(ii+k,jj)
                enddo
            enddo
            !$omp end parallel do
        enddo 
        ...
        do level=0, level_butterfly
            if (level==0) then
                ...
                do i=1, num_groupm
                    ...
                    do ii=1, rank
                        do jj=1, num_vectors
                            ctemp=0
                            do kk=1, mm
                                ctemp=ctemp+matrices_block(blocks,0)%ButterflyU(i)%matrix(kk,ii)*ButterflyVector(0)%blocks(i,1)%matrix(kk,jj)
                            enddo
                ...
                num_groupm=matrices_block(blocks,0)%ButterflyKerl(level_butterfly-level+1)%num_row
                num_groupn=matrices_block(blocks,0)%ButterflyKerl(level_butterfly-level+1)%num_col
                if (num_groupm/=1) then
                    ...
                    do j=1, num_groupn
                        ...
                        do i=1, num_groupm, 2
                            index_i=int((i+1)/2)
                            mm1=size(matrices_block(blocks,0)%ButterflyKerl(level_butterfly-level+1)%blocks(i,j)%matrix,1)
                            mm2=size(matrices_block(blocks,0)%ButterflyKerl(level_butterfly-level+1)%blocks(i+1,j)%matrix,1)
                            nn=size(matrices_block(blocks,0)%ButterflyKerl(level_butterfly-level+1)%blocks(i,j)%matrix,2)
                            ...
                            do jj=1, nn
                                do ii=1, num_vectors
                                    ctemp=0
                                    do kk=1, mm1
                                        ctemp=ctemp+ButterflyVector(level)%blocks(i,index_j)%matrix(kk,ii)*matrices_block(blocks,0)%ButterflyKerl(level_butterfly-level+1)%blocks(i,j)%matrix(kk,jj)
                                    enddo
                                    do kk=1, mm2
                                        ctemp=ctemp+ButterflyVector(level)%blocks(i+1,index_j)%matrix(kk,ii)*matrices_block(blocks,0)%ButterflyKerl(level_butterfly-level+1)%blocks(i+1,j)%matrix(kk,jj)
                                    enddo
                    ...
                    do j=1, num_groupn
                        index_j=int((j+1)/2)
                        nn=size(matrices_block(blocks,0)%ButterflyKerl(level_butterfly-level+1)%blocks(1,j)%matrix,2)
                        mm=size(matrices_block(blocks,0)%ButterflyKerl(level_butterfly-level+1)%blocks(1,j)%matrix,1)
                        ...
                        do ii=1, nn
                            do jj=1, num_vectors
                                ctemp=0                              
                                do kk=1, mm
                                    ctemp=ctemp+ButterflyVector(level)%blocks(1,index_j)%matrix(kk,jj)*matrices_block(blocks,0)%ButterflyKerl(level_butterfly-level+1)%blocks(1,j)%matrix(kk,ii)
                                enddo
                ...
                do j=1, num_blocks
                    ...
                    do ii=1, nn
                        do jj=1, num_vectors
                            ctemp=0
                            do kk=1, rank
                                ctemp=ctemp+ButterflyVector(level+1)%blocks(1,j)%matrix(kk,jj)*matrices_block(blocks,0)%ButterflyV(j)%matrix(ii,kk)
                            enddo
        ...
        do index_j=1, num_blocks
            index_start=index_j-1
            header_n=basis_group(groupn_start+index_start)%head
            tailer_n=basis_group(groupn_start+index_start)%tail
            nn=tailer_n-header_n+1
            k=header_n-header_nn
            !$omp parallel do default(shared) private(ii,jj)
            do ii=1, nn
                do jj=1, num_vectors
                    vectors_block(vector_b)%vector(ii+k,jj)=b*vectors_block(vector_b)%vector(ii+k,jj)+a*ButterflyVector(level_butterfly+2)%blocks(1,index_j)%matrix(ii,jj)
                enddo
            enddo
            !$omp end parallel do
        enddo 
...
end subroutine butterfly_block_MVP
...
! subroutine butterfly_block_MVP_randomized_printCond(blocks,chara,random1,random2,a,b)
   ...
   ! ! use blas95
    ...
    ! integer n, group_m, group_n, group_mm, group_nn, index_i, index_j, na, nb, index_start, num_vectors
    ! integer i, j, ii, jj, level, level_butterfly, groupm_start, groupn_start, index_iijj, index_ij, k, kk, intemp1, intemp2
    ! integer header_m, header_n, tailer_m, tailer_n, vector_inuse, mm, nn, num_blocks, level_define, col_vector
    ! integer rank1, rank2, rank, num_groupm, num_groupn, butterflyB_inuse, header_nn, header_mm, ma, mb
    ! integer vector_a, vector_b, nn1, nn2, level_blocks, mm1, mm2
    ...
    ! type(butterfly_Kerl),allocatable :: ButterflyVector(:)
    ! type(vectorsblock), pointer :: random1, random2
	! integer mm_tot,min_mn,num_row,num_col
    ...
    ! if (chara=='N') then
        ...
        ! do i=1, num_blocks
            ...
            ! do ii=1, nn
                ! do jj=1, num_vectors
                    ! ButterflyVector(0)%blocks(1,i)%matrix(ii,jj)=random1%vector(ii+k,jj)
                ! enddo
            ! enddo
            ! !$omp end parallel do
        ! enddo 
        ...
        ! do index_i=1, num_blocks
            ...
            ! do ii=1, mm
                ! do jj=1, num_vectors
                    ! random2%vector(ii+k,jj)=b*random2%vector(ii+k,jj)+a*ButterflyVector(level_butterfly+2)%blocks(index_i,1)%matrix(ii,jj)
                ! enddo
            ! enddo
            ! !$omp end parallel do
        ! enddo      
		...
		! do level=1, level_butterfly+1		
			...
			! do i=1, num_row
				! do j=1, num_col
					! mm_tot = mm_tot + size(ButterflyVector(level)%blocks(i,j)%matrix,1)
				! enddo
			! enddo
			! allocate (matrixtemp1(mm_tot,num_vectors))
			! min_mn=min(mm_tot,num_vectors)
			...
			! do i=1, num_row
				! do j=1, num_col
					...
					! do jj=1, num_vectors
						! do ii=1, mm
							! matrixtemp1(mm_tot+ii,jj)=ButterflyVector(level)%blocks(i,j)%matrix(ii,jj)
						! enddo
					! enddo					
					! mm_tot = mm + mm_tot
				! enddo
			...
			! write(*,*)'N',level,condition_number
    ...
    ! elseif (chara=='T') then
        ...
        ! do i=1, num_blocks
            ...
            ! do ii=1, mm
                ! do jj=1, num_vectors
                    ! ButterflyVector(0)%blocks(i,1)%matrix(ii,jj)=random1%vector(ii+k,jj)
                ! enddo
            ! enddo
            ! !$omp end parallel do
        ! enddo 
        ...
        ! do level=0, level_butterfly
                ...
                ! do i=1, num_groupm
                    ...
                    ! do ii=1, rank
                        ! do jj=1, num_vectors
                            ...
                            ! do kk=1, mm
                                ! ctemp=ctemp+blocks%ButterflyU(i)%matrix(kk,ii)*ButterflyVector(0)%blocks(i,1)%matrix(kk,jj)
								! ! ! if(ii==1 .and. jj==1 .and. i==1)write(*,*)abs(ctemp),abs(ButterflyVector(0)%blocks(i,1)%matrix(kk,jj))
							! enddo
                            ! ButterflyVector(1)%blocks(i,1)%matrix(ii,jj)=ctemp
                        ! enddo
                    ! enddo
                    ! !$omp end parallel do
                ! enddo                    
					! ! ! write(*,*)'hh111hh',abs(ButterflyVector(0)%blocks(1,1)%matrix(1,1)),abs(ButterflyVector(1)%blocks(1,1)%matrix(1,1))
					! ! ! write(*,*)'hh111hh',abs(ButterflyVector(0)%blocks(2,1)%matrix(1,1)),abs(ButterflyVector(1)%blocks(2,1)%matrix(1,1))
					! ! ! stop
            ! else
                ! num_groupm=blocks%ButterflyKerl(level_butterfly-level+1)%num_row
                ! num_groupn=blocks%ButterflyKerl(level_butterfly-level+1)%num_col
                        ...
                        ! do i=1, num_groupm, 2
                            ...
                            ! do jj=1, nn
                                ! do ii=1, num_vectors
                                    ...
                                    ! do kk=1, mm1
                                        ! ctemp=ctemp+ButterflyVector(level)%blocks(i,index_j)%matrix(kk,ii)*blocks%ButterflyKerl(level_butterfly-level+1)%blocks(i,j)%matrix(kk,jj)
										! ! ! if(ii==1 .and. jj==1)write(*,*)abs(ctemp),abs(ButterflyVector(level)%blocks(i,index_j)%matrix(kk,ii))
									! enddo
                                    ! do kk=1, mm2
                                        ! ctemp=ctemp+ButterflyVector(level)%blocks(i+1,index_j)%matrix(kk,ii)*blocks%ButterflyKerl(level_butterfly-level+1)%blocks(i+1,j)%matrix(kk,jj)
										! ! ! if(ii==1 .and. jj==1)write(*,*)abs(ctemp),abs(ButterflyVector(level)%blocks(i+1,index_j)%matrix(kk,ii))
									! enddo
                                    ! ButterflyVector(level+1)%blocks(index_i,j)%matrix(jj,ii)=ctemp
                                ! enddo
                            ! enddo
							...
							! ! ! write(*,*)'diodio',abs(ButterflyVector(level)%blocks(1,1)%matrix(1,1)),abs(ButterflyVector(level+1)%blocks(1,1)%matrix(1,1))
							! ! ! stop
            ...
            ! if (level==level_butterfly) then
                ...
                ! do j=1, num_blocks
                    ...
                    ! enddo
                    ! !$omp end parallel do
				! ! ! write(*,*)'hhhh',abs(ButterflyVector(level+1)%blocks(1,1)%matrix(1,1)),abs(ButterflyVector(level+2)%blocks(1,1)%matrix(1,1))
				! ! ! stop
                ! enddo
            ! endif
        ! enddo
        ...
        ! do index_j=1, num_blocks
            ...
            ! do ii=1, nn
                ! do jj=1, num_vectors
                    ! random2%vector(ii+k,jj)=b*random2%vector(ii+k,jj)+a*ButterflyVector(level_butterfly+2)%blocks(1,index_j)%matrix(ii,jj)
                ! enddo
            ! enddo
            ! !$omp end parallel do
        ! enddo 
		...
		! do level=1, level_butterfly+1		
			...
			! call gesvd(matrixtemp1,Singular)
			! condition_number=Singular(1)/Singular(min_mn)
			! write(*,*)'T',level,condition_number
			! deallocate (matrixtemp1,Singular)
		! end do	
...
! end subroutine butterfly_block_MVP_randomized_printCond
...
subroutine Butterfly_Partial_MVP_dat_memeff(blocks,chara,level_start,level_end,DimMax,num_vec,VectIn,VectOut)
    
    use MODULE_FILE
    implicit none
    integer DimMax,num_vec
    ...
    integer vector_a, vector_b, nn1, nn2, level_blocks, mm1, mm2, level_end, level_start,levelm
    complex(kind=8) ctemp, a, b
    character chara
	complex(kind=8)::VectIn(:,:),VectOut(:,:)
	integer,allocatable::IndexArray1(:,:,:),IndexArray2(:,:,:)
	type(matrixblock)::blocks
	integer Ni,Nj,indicator,idx1,idx2,idx1_1,middleflag,level_butterfly
    ...
    if (chara=='N') then
        ...
        do level=level_start, level_end
            indicator = mod(indicator+1,2)
			call CountIndexArrayShape(blocks,'N',level,Ni,Nj)
			allocate(IndexArray1(Ni,Nj,2))
			call ComputeRandVectIndexArray(blocks,'N',level,IndexArray1)
	
			call CountIndexArrayShape(blocks,'N',level+1,Ni,Nj)
			allocate(IndexArray2(Ni,Nj,2))
			call ComputeRandVectIndexArray(blocks,'N',level+1,IndexArray2)
			
			if (level==0) then
                num_groupn=num_blocks
               !$omp parallel do default(shared) private(j,rank,nn,idx1,idx2,ii,jj,ctemp,kk)
			   do j=1, num_groupn
                    rank=size(blocks%ButterflyV(j)%matrix,2)
                    nn=size(blocks%ButterflyV(j)%matrix,1)
                    idx1 = IndexArray1(1,j,1)
                    idx2 = IndexArray2(1,j,1)
					...
					if(indicator==1)then
						...
						do ii=1, rank
							do jj=1, num_vec
								ctemp=(0.,0.)
								do kk=1, nn
									ctemp=ctemp+blocks%ButterflyV(j)%matrix(kk,ii)* VectIn(idx1-1+kk,jj)
								enddo
								VectOut(idx2-1+ii,jj)=ctemp
							enddo
						enddo
						! !$omp end parallel do
					else 
						...
						do ii=1, rank
							do jj=1, num_vec
								ctemp=(0.,0.)
								do kk=1, nn
									ctemp=ctemp+blocks%ButterflyV(j)%matrix(kk,ii)* VectOut(idx1-1+kk,jj)
								enddo
								VectIn(idx2-1+ii,jj)=ctemp
							enddo
						enddo
						! !$omp end parallel do						
					end if					
            ...
            elseif (level==level_butterfly+1) then
                num_groupm=num_blocks
                !$omp parallel do default(shared) private(i,rank,mm,ii,jj,ctemp,kk,idx1,idx2)
				do i=1, num_groupm
                    rank=size(blocks%ButterflyU(i)%matrix,2)
                    mm=size(blocks%ButterflyU(i)%matrix,1)
					
					idx1 = IndexArray1(i,1,1)
                    idx2 = IndexArray2(i,1,1)
					if(indicator==1)then
						! !$omp parallel do default(shared) private(ii,jj,kk,ctemp)
						do ii=1, mm
							do jj=1, num_vec
								ctemp=(0.,0.)
								do kk=1, rank
									ctemp=ctemp+blocks%ButterflyU(i)%matrix(ii,kk)*VectIn(idx1-1+kk,jj)
								enddo
								VectOut(idx2-1+ii,jj)=ctemp
							enddo
						enddo
						...
						do ii=1, mm
							do jj=1, num_vec
								ctemp=(0.,0.)
								do kk=1, rank
									ctemp=ctemp+blocks%ButterflyU(i)%matrix(ii,kk)*VectOut(idx1-1+kk,jj)
								enddo
								VectIn(idx2-1+ii,jj)=ctemp
							enddo
						enddo
                ...
                if (num_groupn/=1) then    
                    ...
                    !$omp parallel do default(shared) private(ij,ii,jj,kk,ctemp,i,j,index_i,index_j,nn1,nn2,mm,idx1,idx1_1,idx2)
					do ij=1,num_groupm*(num_groupn/2)
						i = (ij-1)/(num_groupn/2)+1
						j = (mod(ij-1,(num_groupn/2)) + 1)*2-1
						index_i=int((i+1)/2)
						...
						idx1 = IndexArray1(index_i,j,1)
						idx1_1 = IndexArray1(index_i,j+1,1)
						idx2 = IndexArray2(i,index_j,1)
						...
						if(indicator==1)then							
							...
							do ii=1, mm
								do jj=1, num_vec
									ctemp=(0.,0.)
									do kk=1, nn1
										ctemp=ctemp+blocks%ButterflyKerl(level)%blocks(i,j)%matrix(ii,kk)*VectIn(idx1-1+kk,jj)
									enddo
									do kk=1, nn2
										ctemp=ctemp+blocks%ButterflyKerl(level)%blocks(i,j+1)%matrix(ii,kk)*VectIn(idx1_1-1+kk,jj)
									enddo
									VectOut(idx2-1+ii,jj)=ctemp
								enddo
							enddo
							...
							if(level==levelm .and. middleflag==1 .and. level_butterfly>=2)then	
								...
								do ii=1, mm
									do jj=1, num_vec
										VectOut(idx2-1+ii,jj)=VectOut(idx2-1+ii,jj)*blocks%ButterflyMiddle(i,index_j)%matrix(ii,ii)
									enddo
								enddo
								! !$omp end parallel do								
							end if
							...
							do ii=1, mm
								do jj=1, num_vec
									ctemp=(0.,0.)
									do kk=1, nn1
										ctemp=ctemp+blocks%ButterflyKerl(level)%blocks(i,j)%matrix(ii,kk)*VectOut(idx1-1+kk,jj)
									enddo
									do kk=1, nn2
										ctemp=ctemp+blocks%ButterflyKerl(level)%blocks(i,j+1)%matrix(ii,kk)*VectOut(idx1_1-1+kk,jj)
									enddo
									VectIn(idx2-1+ii,jj)=ctemp
								enddo
							enddo
							...
							if(level==levelm .and. middleflag==1 .and. level_butterfly>=2)then		
								...
								do ii=1, mm
									do jj=1, num_vec
										VectIn(idx2-1+ii,jj)=VectIn(idx2-1+ii,jj)*blocks%ButterflyMiddle(i,index_j)%matrix(ii,ii)
									enddo
								enddo
								! !$omp end parallel do		
							end if
			...
			deallocate(IndexArray1,IndexArray2)
			
        enddo      	
		if(indicator==0)then
			VectOut(1:DimMax,1:num_vec) = VectIn(1:DimMax,1:num_vec)
		end if
        ...
        do level=level_start, level_end
			...
			indicator = mod(indicator+1,2)
			call CountIndexArrayShape(blocks,'T',level,Ni,Nj)
			allocate(IndexArray1(Ni,Nj,2))
			call ComputeRandVectIndexArray(blocks,'T',level,IndexArray1)
	
			call CountIndexArrayShape(blocks,'T',level+1,Ni,Nj)
			allocate(IndexArray2(Ni,Nj,2))
			call ComputeRandVectIndexArray(blocks,'T',level+1,IndexArray2)		
					...
					if(indicator==1)then
						...
						do ii=1, rank
							do jj=1, num_vec
								ctemp=(0.,0.)
								do kk=1, mm
									ctemp=ctemp+blocks%ButterflyU(i)%matrix(kk,ii)*VectIn(idx1-1+kk,jj)
								enddo
								VectOut(idx2-1+ii,jj)=ctemp
							enddo
						enddo
						...
						do ii=1, rank
							do jj=1, num_vec
								ctemp=(0.,0.)
								do kk=1, mm
									ctemp=ctemp+blocks%ButterflyU(i)%matrix(kk,ii)*VectOut(idx1-1+kk,jj)
								enddo
								VectIn(idx2-1+ii,jj)=ctemp
							enddo
						enddo
            ...
            elseif (level==level_butterfly+1) then
                num_groupn=num_blocks
                !$omp parallel do default(shared) private(j,rank,nn,ii,jj,ctemp,kk,idx1,idx2)
				do j=1, num_groupn
                    rank=size(blocks%ButterflyV(j)%matrix,2)
                    nn=size(blocks%ButterflyV(j)%matrix,1)
					idx1 = IndexArray1(1,j,1)
                    ...
                    if(indicator==1)then
						...
						do ii=1, nn
							do jj=1, num_vec
								ctemp=(0.,0.)
								do kk=1, rank
									ctemp=ctemp+blocks%ButterflyV(j)%matrix(ii,kk)*VectIn(idx1-1+kk,jj)
								enddo
								VectOut(idx2-1+ii,jj)=ctemp
							enddo
						enddo
						...
						do ii=1, nn
							do jj=1, num_vec
								ctemp=(0.,0.)
								do kk=1, rank
									ctemp=ctemp+blocks%ButterflyV(j)%matrix(ii,kk)*VectOut(idx1-1+kk,jj)
								enddo
								VectIn(idx2-1+ii,jj)=ctemp
							enddo
						enddo
                ...
                num_groupn=blocks%ButterflyKerl(level_butterfly-level+1)%num_col
                if (num_groupm/=1) then                
                    !$omp parallel do default(shared) private(ij,ii,jj,kk,ctemp,i,j,index_i,index_j,mm1,mm2,nn,idx1,idx1_1,idx2)
					do ij=1,num_groupn*(num_groupm/2)
						j = (ij-1)/(num_groupm/2)+1
						i = (mod(ij-1,(num_groupm/2)) + 1)*2-1	
						index_j=int((j+1)/2)
						...
						idx1 = IndexArray1(i,index_j,1)
						idx1_1 = IndexArray1(i+1,index_j,1)
						idx2 = IndexArray2(index_i,j,1)
						if(indicator==1)then
							! !$omp parallel do default(shared) private(ii,jj,kk,ctemp)
							do jj=1, nn
								do ii=1, num_vec
									ctemp=(0.,0.)
									do kk=1, mm1
										ctemp=ctemp+VectIn(idx1-1+kk,ii)*blocks%ButterflyKerl(level_butterfly-level+1)%blocks(i,j)%matrix(kk,jj)
									enddo
									do kk=1, mm2
										ctemp=ctemp+VectIn(idx1_1-1+kk,ii)*blocks%ButterflyKerl(level_butterfly-level+1)%blocks(i+1,j)%matrix(kk,jj)
									enddo
									VectOut(idx2-1+jj,ii)=ctemp
								enddo
							enddo
							! !$omp end parallel do
							...
							if(level_butterfly-level==levelm .and. middleflag==1 .and. level_butterfly>=2)then	
								...
								do jj=1, nn
									do ii=1, num_vec
										VectOut(idx2-1+jj,ii)=VectOut(idx2-1+jj,ii)*blocks%ButterflyMiddle(index_i,j)%matrix(jj,jj)
									enddo
								enddo
								! !$omp end parallel do								
							end if								
							...
							do jj=1, nn
								do ii=1, num_vec
									ctemp=(0.,0.)
									do kk=1, mm1
										ctemp=ctemp+VectOut(idx1-1+kk,ii)*blocks%ButterflyKerl(level_butterfly-level+1)%blocks(i,j)%matrix(kk,jj)
									enddo
									do kk=1, mm2
										ctemp=ctemp+VectOut(idx1_1-1+kk,ii)*blocks%ButterflyKerl(level_butterfly-level+1)%blocks(i+1,j)%matrix(kk,jj)
									enddo
									VectIn(idx2-1+jj,ii)=ctemp
								enddo
							enddo
							! !$omp end parallel do	
							if(level_butterfly-level==levelm .and. middleflag==1 .and. level_butterfly>=2)then	
								...
								do jj=1, nn
									do ii=1, num_vec
										VectIn(idx2-1+jj,ii)=VectIn(idx2-1+jj,ii)*blocks%ButterflyMiddle(index_i,j)%matrix(jj,jj)
									enddo
								enddo
								! !$omp end parallel do								
							end if									
...
end subroutine Butterfly_Partial_MVP_dat_memeff
