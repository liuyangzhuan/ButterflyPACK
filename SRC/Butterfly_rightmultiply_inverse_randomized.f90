...
subroutine Butterfly_Sblock_randomized(level_c,rowblock)
	...
	if(level_butterfly==0)then
		...
		call gemmf90(cascading_factors(Maxlevel_for_blocks+1)%matrices_block_inverse(rowblock)%fullmat, block_o%ButterflyU(1)%matrix, matrixtmp,'N','N',al,be)
		...
		do tt =1,10
			...
			n1 = OMP_get_wtime()
			call Get_Randomized_Vectors_Sblock(level_c,rowblock)        
			n2 = OMP_get_wtime()
...
end subroutine Butterfly_Sblock_randomized
...
subroutine Butterfly_Sblock_randomized_memfree_exact(level_c,rowblock)
	...
	if(level_butterfly==0)then
		...
		do tt =1,20
			...
			n1 = OMP_get_wtime()
			call Reconstruction_LL_Sblock_exact(level_c,rowblock)	
			call Reconstruction_RR_Sblock_exact(level_c,rowblock,error_inout)
			n2 = OMP_get_wtime()
			Time_Reconstruct_forward = Time_Reconstruct_forward + n2-n1
...
end subroutine Butterfly_Sblock_randomized_memfree_exact
...
subroutine Butterfly_Sblock_randomized_symmetric(level_c,rowblock)
	...
	! write(*,*)'here?'
		...
		call Butterfly_Sblock_randomized_memfree(level_c,rowblock,rtmp)
		! write(*,*)'herddde?'
		...
		do tt =1,1
			...
			n1 = OMP_get_wtime()      
			call Get_Randomized_Vectors_Sblock_Memeff(level_c,rowblock,tt-1)
			n2 = OMP_get_wtime()
			Time_Vector_forward = Time_Vector_forward + n2-n1
			
			n1 = OMP_get_wtime()
			call Resolving_Butterfly_Symmetric(block_o%row_group,block_o%col_group)
			call Test_Error_RR_Sblock(level_c,rowblock,error_inout)
			n2 = OMP_get_wtime()
			Time_Reconstruct_forward = Time_Reconstruct_forward + n2-n1
...
end subroutine Butterfly_Sblock_randomized_symmetric
        ...
        ! ! do level=1, level_butterfly
            ...
            ! ! do j=1, num_col
                ! ! do i=1, num_row
                    ! ! allocate (butterfly_block_randomized(1)%ButterflyKerl(level)%blocks(i,j)%matrix(dimension_rank,dimension_rank))
                    ! ! allocate (butterfly_block_randomized(1)%ButterflyKerl_old(level)%blocks(i,j)%matrix(dimension_rank,dimension_rank))
                    ! ! ! ! allocate (butterfly_block_randomized(1)%ButterflyKerl(level)%blocks(i,j)%list(dimension_rank))
				   ! ! ! ! butterfly_block_randomized(1)%ButterflyKerl(level)%blocks(i,j)%list = block%ButterflyKerl(level)%blocks(i,j)%list
				   
				   ! ! !$omp parallel do default(shared) private(ii,jj,a,b,c,d,ctemp)
                    ! ! do jj=1, dimension_rank
                        ! ! do ii=1, dimension_rank
                            ! ! if (level==0) then
								...
								! ! stop
                                ! ! ! ctemp=random_complex_number()*0.01d0
                                ! ! ! butterfly_block_randomized(1)%ButterflyKerl(level)%blocks(i,j)%matrix(ii,jj)=ctemp+butterfly_block_randomized(1)%ButterflyKerl(level)%blocks(i,j)%matrix(ii,jj)
                            ! ! else
                                ! ! ! call random_number(a)
                                ! ! ! call random_number(b)
                                ! ! ! call random_number(c)
                                ...
                                ! ! ! if (d<0.5d0) then
                                    ! ! ! b=-b
                                ! ! ! endif
                                ! ! butterfly_block_randomized(1)%ButterflyKerl(level)%blocks(i,j)%matrix(ii,jj)=random_complex_number()
                                
								! ! ! ! if(butterfly_block_randomized(1)%ButterflyKerl(level)%blocks(i,j)%list(jj)/=0)then
									! ! ! ! butterfly_block_randomized(1)%ButterflyKerl(level)%blocks(i,j)%matrix(ii,jj)=block%ButterflyKerl(level)%blocks(i,j)%matrix(ii,jj)
								! ! ! ! end if
								! ! ! butterfly_block_randomized(1)%ButterflyKerl(level)%blocks(i,j)%matrix(ii,jj)=block%ButterflyKerl(level)%blocks(i,j)%matrix(ii,jj) + random_complex_number()*0.001d0
								! ! ! butterfly_block_randomized(1)%ButterflyKerl(level)%blocks(i,j)%matrix(ii,jj)=block%ButterflyKerl(level)%blocks(i,j)%matrix(ii,jj)
							! ! endif
                            ! ! butterfly_block_randomized(1)%ButterflyKerl_old(level)%blocks(i,j)%matrix(ii,jj)=butterfly_block_randomized(1)%ButterflyKerl(level)%blocks(i,j)%matrix(ii,jj)
                        ! ! enddo
                    ! ! enddo
                    ! ! !$omp end parallel do
                ! ! enddo
            ...
            ! ! do j=1, num_col/2
                ! ! do i=1, num_row/2
                    ! ! allocate (butterfly_block_randomized(1)%ButterflyInv(level)%blocks(i,j)%matrix(2*dimension_rank,2*dimension_rank))
                ! ! enddo
            ! ! enddo
            ! ! call invert_Butterfly_Kernel(level)
        ! ! enddo
        ...
        ! ! ! do level=1, level_butterfly
            ...
            ! ! ! do i=1, num_row
				! ! ! do j=1, num_col
                    ...
                    ! ! ! do jj=1, dimension_rank
                        ! ! ! do ii=1, dimension_rank                           
							! ! ! read(112,*)tmpi,tmpj,a,b
                            ! ! ! butterfly_block_randomized(1)%ButterflyKerl(level)%blocks(i,j)%matrix(ii,jj)=a+junit*b
                            ! ! ! butterfly_block_randomized(1)%ButterflyKerl_old(level)%blocks(i,j)%matrix(ii,jj)=butterfly_block_randomized(1)%ButterflyKerl(level)%blocks(i,j)%matrix(ii,jj)
                        ! ! ! enddo
                    ! ! ! enddo
                ! ! ! enddo
...
subroutine Get_Randomized_Vectors_Sblock_Memeff(level_c,rowblock,kover)
...
! first consider the right multiplied vectors
	...
	do i=1, num_blocks
		...
		allocate(matrixtmp1(nn,dimension_rank))
		call RandomMat(nn,dimension_rank,min(nn,dimension_rank),matrixtmp1,0)
		 !$omp parallel do default(shared) private(ii,jj)
		 do jj=idx_start,idx_start+dimension_rank-1
			 do ii=1, nn
				 RandVectInR(ii+k,jj)=matrixtmp1(ii,jj-idx_start+1) ! random_complex_number()
			 enddo
		 enddo
		 !$omp end parallel do
	...
	do level = Maxlevel_for_blocks+1,level_c+1,-1
		...
		do ii = idx_start_diag,idx_start_diag+N_diag-1
			...
			if(level==Maxlevel_for_blocks+1)then
				call fullmat_block_MVP_randomized_dat(cascading_factors(level)%matrices_block_inverse(ii),'N',idx_end_loc-idx_start_loc+1,num_vectorsR,&
				...
				call butterfly_block_MVP_inverse_dat(level,ii,'N',idx_end_loc-idx_start_loc+1,num_vectorsR,vec_old_loc(1:idx_end_loc-idx_start_loc+1,1:num_vectorsR),vec_new_loc(1:idx_end_loc-idx_start_loc+1,1:num_vectorsR))
...
! next consider the left multiplied vectors
	...
	do i=1, num_blocks
		...
		allocate(matrixtmp1(mm,dimension_rank))
		call RandomMat(mm,dimension_rank,min(mm,dimension_rank),matrixtmp1,0)
		 !$omp parallel do default(shared) private(ii,jj)
		 do jj=idx_start,idx_start+dimension_rank-1
			 do ii=1, mm
				 RandVectInL(ii+k,jj)=matrixtmp1(ii,jj-idx_start+1) !random_complex_number()
			 enddo
		 enddo
		 !$omp end parallel do
		 deallocate(matrixtmp1)				
	...
	do level = level_c+1,Maxlevel_for_blocks+1
		...
		do ii = idx_start_diag,idx_start_diag+N_diag-1
			...
			idx_end_loc = basis_group(groupm_diag)%tail-idx_start_glo+1
			if(level==Maxlevel_for_blocks+1)then
				call fullmat_block_MVP_randomized_dat(cascading_factors(level)%matrices_block_inverse(ii),'T',idx_end_loc-idx_start_loc+1,num_vectorsL,&
				...
				call butterfly_block_MVP_inverse_dat(level,ii,'T',idx_end_loc-idx_start_loc+1,num_vectorsL,vec_old_loc(1:idx_end_loc-idx_start_loc+1,1:num_vectorsL),vec_new_loc(1:idx_end_loc-idx_start_loc+1,1:num_vectorsL))
...
end subroutine Get_Randomized_Vectors_Sblock_Memeff
...
subroutine Resolving_Butterfly_LL_rankcompletion()
   ...
   complex(kind=8), allocatable :: matrixtemp1(:,:),matA(:,:),matB(:,:),matC(:,:),matCtmp(:,:),matinv(:,:),null(:,:),matE(:,:)
   ...
   if (level_butterfly/=0) then
	   ...
	   do level_right=0,level_right_start
		   ...
		   if (level_right==0) then 
			   do j=1, num_blocks
				   ...
				   if(nulldim>0)then
						allocate(null(rank,nulldim),matC(rank,dimension_n),matCtmp(rank,dimension_n),matE(nulldim,dimension_n))
						call RandomMat(nulldim,dimension_n,min(nulldim,dimension_n),matE,1)
						matE = matE * 100000
						call copymatT_omp(butterfly_block_randomized(1)%ButterflyV(j)%matrix,matC,dimension_n,rank)
						call copymatN_omp(random_Block(1)%RandomVectorLL(level_butterfly+1)%blocks(1,j)%null(1:rank,1:nulldim),null,rank,nulldim)
						call gemm_omp(null,matE,matCtmp,rank,nulldim,dimension_n)
						matC = matC + matCtmp
						call copymatT_omp(matC,butterfly_block_randomized(1)%ButterflyV(j)%matrix,rank,dimension_n)
						deallocate(null,matC,matCtmp,matE)
				   end if
				end do
		   elseif (level_right==level_butterfly+1) then
			   ...
			   do j=1, num_col, 2
				   ...
				   do i=1, num_row, 2
					   ...
					   nulldim = random_Block(1)%RandomVectorLL(level_butterfly-level_right+1)%blocks(i,index_j)%nulldim
					   if(nulldim>0)then
							allocate(null(rank,nulldim),matC(rank,rank*2),matCtmp(rank,rank*2),matE(nulldim,rank*2))
							call RandomMat(nulldim,rank*2,min(nulldim,rank*2),matE,1)
							matE = matE * 100000
							...
							call copymatN_omp(random_Block(1)%RandomVectorLL(level_butterfly-level_right+1)%blocks(i,index_j)%null(1:rank,1:nulldim),null,rank,nulldim)
							call gemm_omp(null,matE,matCtmp,rank,nulldim,rank*2)
							matC = matC + matCtmp
					   ...
					   nulldim = random_Block(1)%RandomVectorLL(level_butterfly-level_right+1)%blocks(i+1,index_j)%nulldim
					   if(nulldim>0)then
							allocate(null(rank,nulldim),matC(rank,rank*2),matCtmp(rank,rank*2),matE(nulldim,rank*2))
							call RandomMat(nulldim,rank*2,min(nulldim,rank*2),matE,1)
							matE = matE * 100000
							...
							call copymatN_omp(random_Block(1)%RandomVectorLL(level_butterfly-level_right+1)%blocks(i+1,index_j)%null(1:rank,1:nulldim),null,rank,nulldim)
							call gemm_omp(null,matE,matCtmp,rank,nulldim,rank*2)
							matC = matC + matCtmp
...
end subroutine Resolving_Butterfly_LL_rankcompletion


subroutine Reconstruction_LL_Sblock_exact(level_c,rowblock)
	...
	do ii = 1,Nsub/Nbind
		...
		nth_e = ii*Nbind
		n1 = OMP_get_wtime()
		call Get_Randomized_Vectors_LL_Sblock_exact(level_c,rowblock,nth_s,nth_e,num_vect_sub)
		n2 = OMP_get_wtime()
		time_getvec = time_getvec + n2-n1
...
end subroutine Reconstruction_LL_Sblock_exact
...
subroutine Reconstruction_RR_Sblock_exact(level_c,rowblock,error)
	...
	do ii = 1,Nsub/Nbind
		...
		nth_e = ii*Nbind
		n1 = OMP_get_wtime()
		call Get_Randomized_Vectors_RR_Sblock_exact(level_c,rowblock,nth_s,nth_e,num_vect_sub)
		n2 = OMP_get_wtime()
		time_getvec = time_getvec + n2-n1	
	...
	call Test_Error_RR_Sblock_exact(level_c,rowblock,error)


	
    return
    
end subroutine Reconstruction_RR_Sblock_exact
...
subroutine OneKernel_LL(index_i, index_j,noe,level_right,unique_nth,num_vect_sub,mm,nth,nth_s)
   ...
   ieo = i + 1 - mod(noe,2)
	...
	if(level_right==unique_nth)then
		...
		! if(allocated(blocks%ButterflyInv(level_right)%blocks(index_i,index_j)%matrix))then
			! if(mod(noe,2)==1)then
				! allocate(matinv(nn1+nn2,rank))
				! call RandomMat(nn1+nn2,rank,min(rank,nn1+nn2),matinv,0)
				! deallocate(blocks%ButterflyInv(level_right)%blocks(index_i,index_j)%matrix)
				! allocate(blocks%ButterflyInv(level_right)%blocks(index_i,index_j)%matrix(nn1+nn2,re))
				! call copymatN_omp(matinv,blocks%ButterflyInv(level_right)%blocks(index_i,index_j)%matrix(1:nn1+nn2,rs:re),nn1+nn2,rank)
				! deallocate(matinv)
			! else 
				! allocate(matinv1(nn1+nn2,rs-1))
				! allocate(matinv2(nn1+nn2,rank))	
				! call copymatN_omp(blocks%ButterflyInv(level_right)%blocks(index_i,index_j)%matrix(1:nn1+nn2,1:rs-1),matinv1,nn1+nn2,rs-1)	
				! call RandomMat(nn1+nn2,rank,min(rank,nn1+nn2),matinv2,0)
				! deallocate(blocks%ButterflyInv(level_right)%blocks(index_i,index_j)%matrix)
				! allocate(blocks%ButterflyInv(level_right)%blocks(index_i,index_j)%matrix(nn1+nn2,re))
				! call copymatN_omp(matinv1,blocks%ButterflyInv(level_right)%blocks(index_i,index_j)%matrix(1:nn1+nn2,1:rs-1),nn1+nn2,rs-1)
				! call copymatN_omp(matinv2,blocks%ButterflyInv(level_right)%blocks(index_i,index_j)%matrix(1:nn1+nn2,rs:re),nn1+nn2,rank)
				! deallocate(matinv1,matinv2)
			! end if
		! end if
		
		allocate (matC(rank,nn1+nn2),matA(mm,rank),matinv(nn1+nn2,rank))
		! call copymatN_omp(blocks%ButterflyInv(level_right)%blocks(index_i,index_j)%matrix(1:nn1+nn2,rs:re),matinv,nn1+nn2,rank)
...
! ! subroutine Resolving_Butterfly_LL_new_adaptive(num_vect_sub,nth_s,nth_e,Ng,unique_nth)
   ...
   ! ! integer nth_s,nth_e,unique_nth
   ! ! integer i,j,k,level,num_blocks,num_row,num_col,ii,jj,kk,level_left,level_right,rs,re,rank,level_right_start,level_left_start
   ! ! integer index_i, index_j, mm, nn, iter, vector1, vector2, direction, round, flag
   ! ! real*8 a,b,c,d,norm1,norm2,norm3,norm4,norm1L,norm2L,norm3L,norm4L,norm1R,norm2R,norm3R,norm4R,error,errorL,errorR,rtemp,error0,error1,error2
   ! ! complex(kind=8) ctemp
   ! ! integer kmax
   ...
   ! ! complex(kind=8), allocatable :: matrixtemp(:,:), vectortemp(:), vectorstemp(:,:), tau(:), vectorstemp1(:,:),vectorstemp2(:,:)
   ! ! complex(kind=8), allocatable :: matrixtemp1(:,:),matA(:,:),matB(:,:),matC(:,:),matinv(:,:),matinv1(:,:),matinv2(:,:)
   ! ! integer num_vect_sub,num_vect_subsub,nth,nulldim,ind_r,noe,Ng,level_butterfly,dimension_nn,nn1,nn2,ieo
   ! ! real*8::n1,n2
...
subroutine OneU_RR(i,level_left,unique_nth,num_vect_sub,mm,nth,nth_s)
   ...
   level_butterfly=blocks%level_butterfly 
	...
	if(level_left==unique_nth)then
		...
		! if(allocated(blocks%ButterflyUInv(i)%matrix))deallocate(blocks%ButterflyUInv(i)%matrix)
		...
		! allocate(blocks%ButterflyUInv(i)%matrix(rank,dimension_mm))
		allocate(random_Block(1)%RandomVectorRR(level_butterfly+1)%blocks(i,1)%matrix(rank,num_vect_sub))
		! call RandomMat(rank,dimension_mm,min(rank,dimension_mm),blocks%ButterflyUInv(i)%matrix,0)
		allocate(matC(rank,dimension_mm),matA(mm,rank),matinv(dimension_mm,rank))	
		! call copymatT_omp(blocks%ButterflyUInv(i)%matrix,matinv,rank,dimension_mm)
   ...
   level_butterfly=blocks%level_butterfly 
	...
	if(level_left==unique_nth)then
		if(level_left==level_left_start)then
			...
			! if(allocated(blocks%ButterflyInv(level_left)%blocks(index_i,index_j)%matrix))then
				! if(mod(noe,2)==1)then
					! allocate(matinv(rank,nn1+nn2))
					! call RandomMat(rank,nn1+nn2,min(rank,nn1+nn2),matinv,0)
					! deallocate(blocks%ButterflyInv(level_left)%blocks(index_i,index_j)%matrix)
					! allocate(blocks%ButterflyInv(level_left)%blocks(index_i,index_j)%matrix(re,nn1+nn2))
					! call copymatN_omp(matinv,blocks%ButterflyInv(level_left)%blocks(index_i,index_j)%matrix(rs:re,1:nn1+nn2),rank,nn1+nn2)
					! deallocate(matinv)
				! else 
					! allocate(matinv1(rs-1,nn1+nn2))
					! allocate(matinv2(rank,nn1+nn2))
					! call copymatN_omp(blocks%ButterflyInv(level_left)%blocks(index_i,index_j)%matrix(1:rs-1,1:nn1+nn2),matinv1,rs-1,nn1+nn2)
					! call RandomMat(rank,nn1+nn2,min(rank,nn1+nn2),matinv2,0)
					! deallocate(blocks%ButterflyInv(level_left)%blocks(index_i,index_j)%matrix)
					! allocate(blocks%ButterflyInv(level_left)%blocks(index_i,index_j)%matrix(re,nn1+nn2))
					! call copymatN_omp(matinv1,blocks%ButterflyInv(level_left)%blocks(index_i,index_j)%matrix(1:rs-1,1:nn1+nn2),rs-1,nn1+nn2)
					! call copymatN_omp(matinv2,blocks%ButterflyInv(level_left)%blocks(index_i,index_j)%matrix(rs:re,1:nn1+nn2),rank,nn1+nn2)
					! deallocate(matinv1,matinv2)
				! end if
			! end if
			allocate(matC(rank,nn1+nn2),matA(mm,rank),matinv(nn1+nn2,rank))
			call copymatT_OMP(blocks%KerInv(rs:re,1:nn1+nn2),matinv,rank,nn1+nn2)															   
			! call copymatT_omp(blocks%ButterflyInv(level_left)%blocks(index_i,index_j)%matrix(rs:re,1:nn1+nn2),matinv,rank,nn1+nn2)
			if(isnan(fnorm(matB,mm,nn1+nn2)) .or. isnan(fnorm(matinv,nn1+nn2,rank)))then
...
! subroutine Resolving_Butterfly_RR_new_adaptive(num_vect_sub,nth_s,nth_e,Ng,unique_nth)
   ...
   ! complex(kind=8), allocatable :: matrixtemp(:,:), vectortemp(:), vectorstemp(:,:), tau(:), vectorstemp1(:,:),vectorstemp2(:,:)
   ! complex(kind=8), allocatable :: matrixtemp1(:,:),matA(:,:),matB(:,:),matC(:,:),matinv(:,:),matinv1(:,:),matinv2(:,:)
   ! integer num_vect_sub,num_vect_subsub,nth,nulldim,ind_r,ind_c,noe,Ng,nth_s,nth_e,rs,re,dimension_mm,nn1,nn2,jeo
   ! real*8::n1,n2
   ! integer::kmax,unique_nth
						   ...
						   if(index_i==ind_r)then
								...
								if(mod(noe,2)==1)then
									...
									! ! else 
										...
										! call copymatN_omp(butterfly_block_randomized(1)%ButterflyInv(level_right)%blocks(index_i,index_j)%matrix(1:2*rank,1:rank),matinv,2*rank,rank)
									...
									! call copymatN_omp(butterfly_block_randomized(1)%ButterflyInv(level_right)%blocks(index_i,index_j)%matrix(1:2*rank,1+rank:2*rank),matinv,2*rank,rank)
										call gemm_omp(matB,matinv,matA,mm,2*rank,rank)
			...
			if (level_left==level_butterfly+1) then
			   do nth=nth_s,nth_e
				   do i=1, num_blocks
					   ...
					   ! call copymatT_OMP(butterfly_block_randomized(1)%ButterflyUInv(i)%matrix,matinv,rank,dimension_m)
								...
								! ! ! ! ! else 
									if(mod(noe,2)==1)then
										if(level_left==level_left_start)then
											...
											! call copymatT_OMP(butterfly_block_randomized(1)%ButterflyInv(level_left)%blocks(index_i,index_j)%matrix(1:rank,1:2*rank),matinv,rank,2*rank)
										...
										if(level_left==level_left_start)then
											...
											! call copymatT_OMP(butterfly_block_randomized(1)%ButterflyInv(level_left)%blocks(index_i,index_j)%matrix(1+rank:2*rank,1:2*rank),matinv,rank,2*rank)
											call gemm_omp(matB,matinv,matA,mm,2*rank,rank)
					   ...
					   do i=1, num_row, 2
						   ...
						   if(index_i==ind_r)then
								...
								! ! ! if(level_right==level_right_start)then
									! ! ! if(mod(noe,2)==1)then
										! ! ! call copymatT_omp(random_Block(1)%RandomVectorLL(level_butterfly-level_right+1)%blocks(i,index_j)%matrix(1:rank,(nth-nth_s)*mm+1:(nth-nth_s+1)*mm),matA,rank,mm)
										! ! ! ! write(*,*)level_butterfly-level_right+1,i,index_j,fnorm(matA,mm,rank),fnorm(random_Block(1)%RandomVectorLL(level_butterfly-level_right+1)%blocks(i,index_j)%matrix(1:rank,(nth-nth_s)*mm+1:(nth-nth_s+1)*mm),rank,mm)
										! ! ! call LeastSquare(mm,rank,2*rank,matA,matB,matC,LS_tolerance)
										! ! ! call copymatN_omp(matC(1:rank,1:rank),butterfly_block_randomized(1)%ButterflyKerl(level_right)%blocks(i,j)%matrix,rank,rank)
										! ! ! call copymatN_omp(matC(1:rank,rank+1:2*rank),butterfly_block_randomized(1)%ButterflyKerl(level_right)%blocks(i,j+1)%matrix,rank,rank)								
									! ! ! else 
										! ! ! call copymatT_omp(random_Block(1)%RandomVectorLL(level_butterfly-level_right+1)%blocks(i+1,index_j)%matrix(1:rank,(nth-nth_s)*mm+1:(nth-nth_s+1)*mm),matA,rank,mm)
										! ! ! ! write(*,*)level_butterfly-level_right+1,i+1,index_j,fnorm(matA,mm,rank),fnorm(random_Block(1)%RandomVectorLL(level_butterfly-level_right+1)%blocks(i+1,index_j)%matrix(1:rank,(nth-nth_s)*mm+1:(nth-nth_s+1)*mm),rank,mm)
										! ! ! call LeastSquare(mm,rank,2*rank,matA,matB,matC,LS_tolerance)
										! ! ! call copymatN_omp(matC(1:rank,1:rank),butterfly_block_randomized(1)%ButterflyKerl(level_right)%blocks(i+1,j)%matrix,rank,rank)
										! ! ! call copymatN_omp(matC(1:rank,rank+1:2*rank),butterfly_block_randomized(1)%ButterflyKerl(level_right)%blocks(i+1,j+1)%matrix,rank,rank)										
									! ! ! end if
								...
								! ! ! if(mod(noe,2)==1)then
									! ! ! write(*,*)nth,level_right,i,j,fnorm(butterfly_block_randomized(1)%ButterflyKerl(level_right)%blocks(i,j)%matrix,rank,rank),fnorm(butterfly_block_randomized(1)%ButterflyKerl(level_right)%blocks(i,j+1)%matrix,rank,rank),'L'
								! ! ! else
									! ! ! write(*,*)nth,level_right,i,j,fnorm(butterfly_block_randomized(1)%ButterflyKerl(level_right)%blocks(i+1,j)%matrix,rank,rank),fnorm(butterfly_block_randomized(1)%ButterflyKerl(level_right)%blocks(i+1,j+1)%matrix,rank,rank),'L'
								! ! ! end if
...
subroutine Resolving_Butterfly_RR(num_vect_sub,nth_s,nth_e,Ng)
   ...
   if (level_butterfly/=0) then
	   ...
	   do level_left = level_butterfly+1,level_left_start,-1
			! kmax = ceiling_safe(rank/dble(2**(level_left-level_left_start)))+1
			kmax = rank
			if (level_left==level_butterfly+1) then
			   do nth=nth_s,nth_e
				   ...
				   do j=1, num_col, 2
					   ...
					   do i=1, num_row, 2
						   ...
						   if(index_j==ind_c)then
								...
								! ! if(mod(noe,2)==1)then
									! ! write(*,*)nth,level_left,i,j,fnorm(butterfly_block_randomized(1)%ButterflyKerl(level_left)%blocks(i,j)%matrix,rank,rank),fnorm(butterfly_block_randomized(1)%ButterflyKerl(level_left)%blocks(i+1,j)%matrix,rank,rank),'R'
								! ! else
									! ! write(*,*)nth,level_left,i,j,fnorm(butterfly_block_randomized(1)%ButterflyKerl(level_left)%blocks(i,j+1)%matrix,rank,rank),fnorm(butterfly_block_randomized(1)%ButterflyKerl(level_left)%blocks(i+1,j+1)%matrix,rank,rank),'R'
								! ! end if
...
! Be careful with RandomMat in the target butterfly construction, 
! if all singular values are the same, the SVD of the same middle level matrix block may not be unique due to rounding erros
subroutine Resolving_Butterfly_Symmetric(group_m_root,group_n_root)
   ...
   implicit none
    ...
    integer blocks_idx, i, j,level_butterflyL,level_butterflyR, num_blocks, k, attempt
    integer group_m_root,group_n_root,group_m, group_n, mm, nn,mn, index_i, index_j,index_i_m, index_j_m,index_i_loc, index_j_loc, ii, jj,nn1,nn2,mm1,mm2,idxs_m,idxs_n
	...
	complex(kind=8), allocatable :: tau_Q(:),matRcol(:,:),matZRcol(:,:),matRrow(:,:),matZcRrow(:,:)
	real*8,allocatable :: Singular(:)
	integer flag,tmpi,tmpj
	real*8:: a,b
	integer num_col,num_row
	integer NsubL,NsubR,NgL,NgR,level_butterfly,rank_reconstruction

    level_butterfly = butterfly_block_randomized(1)%level_butterfly
    rank_reconstruction = butterfly_block_randomized(1)%dimension_rank
	

	
	
	butterfly_block_randomized(1)%rankmax = -100000
	butterfly_block_randomized(1)%rankmin = 100000	
	
	num_blocks=2**level_butterfly

    allocate(butterfly_block_randomized(1)%ButterflyU(num_blocks))
    allocate(butterfly_block_randomized(1)%ButterflyV(num_blocks))
    if (level_butterfly/=0) then
        allocate(butterfly_block_randomized(1)%ButterflyKerl(level_butterfly))
    endif
    
	call assert(level_butterfly>=2,'L must be larger than 1')
		
	do level=1, level_butterfly
		butterfly_block_randomized(1)%ButterflyKerl(level)%num_row=2**level
		butterfly_block_randomized(1)%ButterflyKerl(level)%num_col=2**(level_butterfly-level+1)
		allocate (butterfly_block_randomized(1)%ButterflyKerl(level)%blocks(2**level,2**(level_butterfly-level+1)))
	end do
	...
	allocate(butterfly_block_randomized(1)%ButterflyMiddle(2**levelm,2**(level_butterfly-levelm)))
	
	NsubL = 2**levelm
	NsubR = 2**(level_butterfly-levelm)	
	NgL = 2**level_butterfly/NsubL
	NgR = 2**level_butterfly/NsubR
	
	! construct the middle level and the left half
	
	do index_i_m=1, NsubL
		level_loc = 0
		index_i_loc = 1
		allocate(ButterflyP_old%blocks(2**(level_loc+1),2**(level_butterflyL-level_loc)))
		
		do index_j_m=1, NsubR
			index_j_loc = index_j_m
			  
			group_m=group_m_root*2**levelm-1+index_i_m
			group_n=group_n_root*2**(level_butterfly-levelm)-1+index_j_m
			mm=basis_group(group_m)%tail-basis_group(group_m)%head+1
			nn=basis_group(group_n)%tail-basis_group(group_n)%head+1
			rmax = rank_reconstruction
			idxs_m = basis_group(group_m)%head - basis_group(group_m_root)%head + 1
			idxs_n = basis_group(group_n)%head - basis_group(group_n_root)%head + 1
			
			allocate(matRcol(nn,rmax))
			matRcol = RandVectInR(idxs_n:idxs_n+nn-1,(index_j_m-1)*rmax+1:index_j_m*rmax)
			allocate(matZRcol(mm,rmax))
			matZRcol = RandVectOutR(idxs_m:idxs_m+mm-1,(index_j_m-1)*rmax+1:index_j_m*rmax)
			
			allocate(matRrow(mm,rmax))
			matRrow = RandVectInL(idxs_m:idxs_m+mm-1,(index_i_m-1)*rmax+1:index_i_m*rmax)
			matRrow = conjg(matRrow)
			allocate(matZcRrow(nn,rmax))
			matZcRrow = RandVectOutL(idxs_n:idxs_n+nn-1,(index_i_m-1)*rmax+1:index_i_m*rmax)
			...
			! write(111,*)abs(matRcol(1,1)),abs(matZRcol(1,1)),abs(matRrow(1,1)),abs(matZcRrow(1,1))
			
			call RandomizedSVD(matRcol,matZRcol,matRrow,matZcRrow,matU,matV,Singular,mm,nn,rmax,rank,LS_tolerance,SVD_tolerance_factor)				
			deallocate(matRcol,matZRcol,matRrow,matZcRrow)
			if(level_butterfly>=9)write(112,*)rank
			...
			allocate(butterfly_block_randomized(1)%ButterflyMiddle(index_i_m,index_j_m)%matrix(rank,rank))
			butterfly_block_randomized(1)%ButterflyMiddle(index_i_m,index_j_m)%matrix = 0
			do ii=1,rank
				butterfly_block_randomized(1)%ButterflyMiddle(index_i_m,index_j_m)%matrix(ii,ii) = 1d0/Singular(ii)
			end do
			...
			! write(111,*)abs(ButterflyP_old%blocks(2*index_i_loc-1,index_j_loc)%matrix(1,1))
		...
		do level_loc = 1,level_butterflyL
			...
			do index_i_loc=1, 2**level_loc
				do index_j_loc=1, 2**(level_butterflyL-level_loc)
					...
					group_m=group_m_root*2**level-1+index_i
					mm=basis_group(group_m)%tail-basis_group(group_m)%head+1
					...
					nn = nn1+nn2
					! write(*,*)shape(ButterflyP_old%blocks(index_i_loc,2*index_j_loc-1)%matrix),shape(ButterflyP_old%blocks(index_i_loc,2*index_j_loc)%matrix)
					...
					rankmax_for_butterfly(level_loc)=max(rank,rankmax_for_butterfly(level_loc))
					rankmin_for_butterfly(level_loc)=min(rank,rankmin_for_butterfly(level_loc))
					butterfly_block_randomized(1)%rankmax = max(butterfly_block_randomized(1)%rankmax,rank)
					butterfly_block_randomized(1)%rankmin = min(butterfly_block_randomized(1)%rankmin,rank)
					...
					if(level_loc/=level_butterflyL)then
						...
						allocate (butterfly_block_randomized(1)%ButterflyU(index_i)%matrix(mm,rank))
						!$omp parallel do default(shared) private(i,j)
						do j=1, rank
							do i=1, mm
								butterfly_block_randomized(1)%ButterflyU(index_i)%matrix(i,j)=mat_tmp(i,j)
								! butterfly_block_randomized(1)%ButterflyU(index_i)%matrix(i,j)=random_complex_number()
							enddo
						enddo
						!$omp end parallel do
					...
					allocate (butterfly_block_randomized(1)%ButterflyKerl(level)%blocks(index_i,2*index_j-1)%matrix(rank,nn1))
					allocate (butterfly_block_randomized(1)%ButterflyKerl(level)%blocks(index_i,2*index_j)%matrix(rank,nn2))
					
					!$omp parallel do default(shared) private(i,j)
					do i=1, rank
						do j=1, nn1
							butterfly_block_randomized(1)%ButterflyKerl(level)%blocks(index_i,2*index_j-1)%matrix(i,j)=VV(i,j)
							! butterfly_block_randomized(1)%ButterflyKerl(level)%blocks(index_i,2*index_j-1)%matrix(i,j)=random_complex_number()
						enddo
					enddo
					!$omp end parallel do						
					!$omp parallel do default(shared) private(i,j)
					do i=1, rank
						do j=1, nn2
							butterfly_block_randomized(1)%ButterflyKerl(level)%blocks(index_i,2*index_j)%matrix(i,j)=VV(i,j+nn1)
							! butterfly_block_randomized(1)%ButterflyKerl(level)%blocks(index_i,2*index_j)%matrix(i,j)=random_complex_number()
						enddo
					enddo
					!$omp end parallel do						
		...
		do level_loc = 1,level_butterflyR
			...
			do index_i_loc=1, 2**(level_butterflyR-level_loc)
				do index_j_loc=1,2**level_loc 
					...
					group_n=group_n_root*2**(level_butterfly-level+1)-1+index_j
					nn=basis_group(group_n)%tail-basis_group(group_n)%head+1		
					! nn = size(ButterflyP_old%blocks(2*index_i_loc-1,index_j_loc)%matrix,2)
					...
					if(level_loc/=level_butterflyR)then
						...
						allocate (butterfly_block_randomized(1)%ButterflyV(index_j)%matrix(nn,rank))
						!$omp parallel do default(shared) private(i,j)
						do j=1, rank
							do i=1, nn
								butterfly_block_randomized(1)%ButterflyV(index_j)%matrix(i,j)=mat_tmp(j,i)
								! butterfly_block_randomized(1)%ButterflyU(index_i)%matrix(i,j)=random_complex_number()
							enddo
						enddo
						!$omp end parallel do
					...
					allocate (butterfly_block_randomized(1)%ButterflyKerl(level)%blocks(2*index_i-1,index_j)%matrix(mm1,rank))
					allocate (butterfly_block_randomized(1)%ButterflyKerl(level)%blocks(2*index_i,index_j)%matrix(mm2,rank))
					
					!$omp parallel do default(shared) private(i,j)
					do i=1, mm1
						do j=1, rank
							butterfly_block_randomized(1)%ButterflyKerl(level)%blocks(2*index_i-1,index_j)%matrix(i,j)=UU(i,j)
							! butterfly_block_randomized(1)%ButterflyKerl(level)%blocks(index_i,2*index_j-1)%matrix(i,j)=random_complex_number()
						enddo
					enddo
					!$omp end parallel do						
					...
					do i=1, mm2
						do j=1, rank
							butterfly_block_randomized(1)%ButterflyKerl(level)%blocks(2*index_i,index_j)%matrix(i,j)=UU(i+mm1,j)
							! butterfly_block_randomized(1)%ButterflyKerl(level)%blocks(index_i,2*index_j)%matrix(i,j)=random_complex_number()
						enddo
					enddo
					!$omp end parallel do						
	...
	deallocate(RandVectInR,RandVectInL,RandVectOutR,RandVectOutL)

    return

end subroutine Resolving_Butterfly_Symmetric





subroutine Resolving_Butterfly_LL_Nlog2N(num_vect_sub,unique_nth)
   ...
   integer unique_nth
   integer i,j,k,level,num_blocks,num_row,num_col,ii,jj,mm,kk,level_left,level_right, rs,re,rank,level_right_start,level_left_start
   integer index_i, index_j, iter, vector1, vector2, direction, round, flag,mn
   real*8 a,b,c,d,norm1,norm2,norm3,norm4,norm1L,norm2L,norm3L,norm4L,norm1R,norm2R,norm3R,norm4R,error,errorL,errorR,rtemp,error0,error1,error2
   complex(kind=8) ctemp
   integer kmax
   ...
   integer, allocatable :: ipiv(:), kernel_selection(:)
   real*8, allocatable :: Singular(:),Singular1(:)
   complex(kind=8), allocatable :: matrixtemp(:,:), vectortemp(:), vectorstemp(:,:), tau(:), vectorstemp1(:,:),vectorstemp2(:,:)
   complex(kind=8), allocatable :: matrixtemp1(:,:),matA(:,:),matB(:,:),matB1(:,:),matBtmp(:,:),matC(:,:),matinv(:,:),matinv1(:,:),matinv2(:,:),UU(:,:),VV(:,:),UU1(:,:),VV1(:,:),mattmp2(:,:),mattmp1(:,:)
   integer num_vect_sub,num_vect_subsub,nth,ind_r,noe,Ng,dimension_nn,dimension_mm,nn1,nn2,rr1,rr2,rr,level_butterfly
   real*8::n1,n2
   type(butterflyblock_randomized),pointer :: blocks
   real*8:: ctemp1,ctemp2

   blocks => butterfly_block_randomized(1)   
   level_butterfly=blocks%level_butterfly 
   num_blocks=2**level_butterfly
   num_vect_subsub = num_vect_sub  
   
	
   if (level_butterfly/=0) then
       mm=num_vect_sub !rank

		if(unique_nth==1)then
			do j=1, num_blocks
			   dimension_nn=size(blocks%butterflyV(j)%matrix,1)
			   ! dimension_nn = dimension_n
			   allocate(matB(mm,dimension_nn))
			   call copymatT_omp(random_Block(1)%RandomVectorLL(level_butterfly+2)%blocks(1,j)%matrix,matB,dimension_nn,mm)
			   call GetRank(mm,dimension_nn,matB,rank,SVD_tolerance_forward*1d0)
			   if(allocated(blocks%butterflyV(j)%matrix))deallocate(blocks%butterflyV(j)%matrix)
			   if(allocated(blocks%ButterflyVInv(j)%matrix))deallocate(blocks%ButterflyVInv(j)%matrix)
			   if(allocated(random_Block(1)%RandomVectorLL(level_butterfly+1)%blocks(1,j)%matrix))deallocate(random_Block(1)%RandomVectorLL(level_butterfly+1)%blocks(1,j)%matrix)
			   allocate(blocks%butterflyV(j)%matrix(dimension_nn,rank))
			   ...
			   call GeneralInverse(rank,dimension_nn,matC,matinv,LS_tolerance)
			   call copymatT_omp(matinv,blocks%ButterflyVInv(j)%matrix,dimension_nn,rank)						   
			   deallocate(matB,matC,matA,matinv)	
			end do						   
		end if
	   ...
	   do level_right=0,unique_nth !level_right_start
		   if (level_right==0) then
			   do j=1, num_blocks
				   ...
				   call copymatT_omp(matA,random_Block(1)%RandomVectorLL(level_butterfly+1)%blocks(1,j)%matrix,mm,rank)					   
				   	
				   call GetRank(mm,dimension_nn,matB(1:mm,1:dimension_nn),rank,SVD_tolerance_forward*1d0)
				   deallocate(matB,matA,matinv)	
				end do				
		   else
			   ...
			   num_col=blocks%ButterflyKerl(level_right)%num_col
				...
				do index_j = 1,num_col/2
					j = index_j*2-1
					do index_i = 1,num_row/2
						i=index_i*2-1
						...
						if(level_right==unique_nth)then
							...
							allocate (matB(mm,nn1+nn2))
							call copymatT_omp(random_Block(1)%RandomVectorLL(level_butterfly-level_right+2)%blocks(index_i,j)%matrix,matB(1:mm,1:nn1),nn1,mm)
							call copymatT_omp(random_Block(1)%RandomVectorLL(level_butterfly-level_right+2)%blocks(index_i,j+1)%matrix,matB(1:mm,1+nn1:nn2+nn1),nn2,mm)

							
							call GetRank(mm,nn1+nn2,matB,rr,SVD_tolerance_forward*1d-3)
							write(*,*)rr,'rr'

							! ! ! allocate (matB1(mm,nn1+nn2))
							! ! ! matB1 = matB
							
							
							! ! mn=min(mm/2,nn1+nn2)
							! ! allocate (UU(mm/2,mn),VV(mn,nn1+nn2),Singular(mn))
							! ! call SVD_Truncate(matB(1:mm/2,1:nn1+nn2),mm/2,nn1+nn2,mn,UU,VV,Singular,SVD_tolerance_forward*1d0,rr1)			
							! ! allocate (UU1(mm/2,mn),VV1(mn,nn1+nn2),Singular1(mn))
							! ! call SVD_Truncate(matB(1+mm/2:mm,1:nn1+nn2),mm/2,nn1+nn2,mn,UU1,VV1,Singular1,SVD_tolerance_forward*1d0,rr2)								
							! ! ! write(*,*)rr1,rr2
							! ! allocate(matA(mm,rr1+rr2))
							! ! allocate(matC(rr1+rr2,nn1+nn2))
							! ! matA = 0
							! ! matA(1:mm/2,1:rr1) = UU(1:mm/2,1:rr1)
							! ! matA(1+mm/2:mm,1+rr1:rr1+rr2) = UU1(1:mm/2,1:rr2)
							
							
							allocate (matBtmp(mm,nn1+nn2))
							
							allocate(mattmp1(mm/2,nn1+nn2))
							call gemmHN_omp(Basis_split(1:mm,1:mm/2),matB(1:mm,1:nn1+nn2),mattmp1,mm/2,mm,nn1+nn2)
							allocate(mattmp2(mm,nn1+nn2))
							call gemm_omp(Basis_split(1:mm,1:mm/2),mattmp1,mattmp2,mm,mm/2,nn1+nn2)
							
							mn=min(mm,nn1+nn2)
							allocate (UU(mm,mn),VV(mn,nn1+nn2),Singular(mn))
							call SVD_Truncate(mattmp2,mm,nn1+nn2,mn,UU,VV,Singular,SVD_tolerance_forward*1d0,rr1)
							matBtmp = mattmp2
							deallocate(mattmp1)
							deallocate(mattmp2)
							...
							allocate(mattmp1(mm/2,nn1+nn2))
							call gemmHN_omp(Basis_split(1:mm,1+mm/2:mm),matB(1:mm,1:nn1+nn2),mattmp1,mm/2,mm,nn1+nn2)
							allocate(mattmp2(mm,nn1+nn2))
							call gemm_omp(Basis_split(1:mm,1+mm/2:mm),mattmp1,mattmp2,mm,mm/2,nn1+nn2)
							
							mn=min(mm,nn1+nn2)
							allocate (UU1(mm,mn),VV1(mn,nn1+nn2),Singular1(mn))
							call SVD_Truncate(mattmp2,mm,nn1+nn2,mn,UU1,VV1,Singular1,SVD_tolerance_forward*1d0,rr2)
							matB = matBtmp + mattmp2
							deallocate(mattmp1)
							deallocate(mattmp2)	
							deallocate(matBtmp)
							...
							matA = 0
							matA(:,1:rr1) = UU(:,1:rr1)
							matA(:,1+rr1:rr1+rr2) = UU1(:,1:rr2)

							
							write(*,*)mm,rr1,rr2,nn1,nn2
							
							
							
							call LeastSquare(mm,rr1+rr2,nn1+nn2,matA,matB,matC,LS_tolerance*1d0)
							
							! ! ! allocate(matinv(nn1+nn2,rr1+rr2))
							! ! ! call GeneralInverse(rr1+rr2,nn1+nn2,matC,matinv,LS_tolerance)	
							
							! ! ! call gemm_omp(matB1,matinv,matA,mm,nn1+nn2,rr1+rr2)
							! ! ! call LeastSquare(mm,rr1+rr2,nn1+nn2,matA,matB1,matC,LS_tolerance)
							! ! ! deallocate(matinv,matB1)
							
							
							
							allocate(random_Block(1)%RandomVectorLL(level_butterfly-level_right+1)%blocks(i,index_j)%matrix(rr1,mm))
							allocate(random_Block(1)%RandomVectorLL(level_butterfly-level_right+1)%blocks(i+1,index_j)%matrix(rr2,mm))
							
							allocate(blocks%ButterflyKerl(level_right)%blocks(i,j)%matrix(rr1,nn1))
							allocate(blocks%ButterflyKerl(level_right)%blocks(i,j+1)%matrix(rr1,nn2))							
							allocate(blocks%ButterflyKerl(level_right)%blocks(i+1,j)%matrix(rr2,nn1))
							allocate(blocks%ButterflyKerl(level_right)%blocks(i+1,j+1)%matrix(rr2,nn2))							
							
							call copymatN_omp(matC(1:rr1,1:nn1),blocks%ButterflyKerl(level_right)%blocks(i,j)%matrix,rr1,nn1)		
							call copymatN_omp(matC(1:rr1,1+nn1:nn1+nn2),blocks%ButterflyKerl(level_right)%blocks(i,j+1)%matrix,rr1,nn2)		
							call copymatN_omp(matC(1+rr1:rr1+rr2,1:nn1),blocks%ButterflyKerl(level_right)%blocks(i+1,j)%matrix,rr2,nn1)		
							call copymatN_omp(matC(1+rr1:rr1+rr2,1+nn1:nn1+nn2),blocks%ButterflyKerl(level_right)%blocks(i+1,j+1)%matrix,rr2,nn2)		
							
							allocate(matinv(nn1+nn2,rr1+rr2))
							call GeneralInverse(rr1+rr2,nn1+nn2,matC,matinv,LS_tolerance*1d0)						
							
							allocate(blocks%ButterflyInv(level_right)%blocks(index_i,index_j)%matrix(nn1+nn2,rr1+rr2))
							call copymatN_omp(matinv,blocks%ButterflyInv(level_right)%blocks(index_i,index_j)%matrix,nn1+nn2,rr1+rr2)
							
							deallocate(matB,matC,matA,matinv)	
							deallocate(UU,VV,Singular,UU1,VV1,Singular1)
						else
							nn1 = size(random_Block(1)%RandomVectorLL(level_butterfly-level_right+2)%blocks(index_i,j)%matrix,1)
							nn2 = size(random_Block(1)%RandomVectorLL(level_butterfly-level_right+2)%blocks(index_i,j+1)%matrix,1)						
							rr1 = size(blocks%ButterflyKerl(level_right)%blocks(i,j)%matrix,1)
							rr2 = size(blocks%ButterflyKerl(level_right)%blocks(i+1,j)%matrix,1)
							
							allocate (matB(mm,nn1+nn2),matC(rr1+rr2,nn1+nn2),matA(mm,rr1+rr2),matinv(nn1+nn2,rr1+rr2))
							call copymatT_omp(random_Block(1)%RandomVectorLL(level_butterfly-level_right+2)%blocks(index_i,j)%matrix,matB(1:mm,1:nn1),nn1,mm)
							call copymatT_omp(random_Block(1)%RandomVectorLL(level_butterfly-level_right+2)%blocks(index_i,j+1)%matrix,matB(1:mm,1+nn1:nn2+nn1),nn2,mm)
															
							call copymatN_omp(blocks%ButterflyInv(level_right)%blocks(index_i,index_j)%matrix,matinv,nn1+nn2,rr1+rr2)
							call gemm_omp(matB,matinv,matA,mm,nn1+nn2,rr1+rr2)
							call copymatT_omp(matA(1:mm,1:rr1),random_Block(1)%RandomVectorLL(level_butterfly-level_right+1)%blocks(i,index_j)%matrix,mm,rr1)
							call copymatT_omp(matA(1:mm,1+rr1:rr1+rr2),random_Block(1)%RandomVectorLL(level_butterfly-level_right+1)%blocks(i+1,index_j)%matrix,mm,rr2)
							deallocate(matB,matC,matA,matinv)								
						end if
				   enddo
			   enddo
	   ...
	   if(unique_nth==level_butterfly)then

			level_right = level_butterfly
			num_row=blocks%ButterflyKerl(level_right)%num_row
			num_col=blocks%ButterflyKerl(level_right)%num_col
			...
			do i=1, num_blocks
				dimension_mm=size(blocks%butterflyU(i)%matrix,1)
			   ! dimension_mm = dimension_m
				rank = size(random_Block(1)%RandomVectorLL(1)%blocks(i,1)%matrix,1)
				allocate(matB(mm,rank),matC(dimension_mm,rank),matA(mm,dimension_mm))
				call copymatT_omp(random_Block(1)%RandomVectorLL(1)%blocks(i,1)%matrix,matB,rank,mm)
				call copymatT_omp(random_Block(1)%RandomVectorLL(0)%blocks(i,1)%matrix,matA,dimension_mm,mm)
				
				allocate(mattmp1(mm/2,rank))
				allocate(mattmp2(mm,rank))
				
				! if(i==1)then
					call gemmHN_omp(Basis_split(1:mm,1:mm/2),matB(1:mm,1:rank),mattmp1,mm/2,mm,rank)
					call gemm_omp(Basis_split(1:mm,1:mm/2),mattmp1,mattmp2,mm,mm/2,rank)
					ctemp1 = fnorm(mattmp2,mm,rank)
					! call LeastSquare(mm,dimension_mm,rank,matA,mattmp2,matC,LS_tolerance)
				! else 
					call gemmHN_omp(Basis_split(1:mm,1+mm/2:mm),matB(1:mm,1:rank),mattmp1,mm/2,mm,rank)
					call gemm_omp(Basis_split(1:mm,1+mm/2:mm),mattmp1,mattmp2,mm,mm/2,rank)
					ctemp2 = fnorm(mattmp2,mm,rank)
					write(*,*)ctemp1,'Basis1',ctemp2,'Basis2'
					! call LeastSquare(mm,dimension_mm,rank,matA,mattmp2,matC,LS_tolerance)
				! end if
				deallocate(mattmp1)
				deallocate(mattmp2)
				
				call LeastSquare(mm,dimension_mm,rank,matA,matB,matC,LS_tolerance)
				if(allocated(blocks%butterflyU(i)%matrix))deallocate(blocks%butterflyU(i)%matrix)
				allocate(blocks%butterflyU(i)%matrix(dimension_mm,rank))
				call copymatN_omp(matC,blocks%butterflyU(i)%matrix,dimension_mm,rank)
				allocate(matinv(rank,dimension_mm))
				allocate(blocks%butterflyUInv(i)%matrix(rank,dimension_mm))
				call GeneralInverse(dimension_mm,rank,matC,matinv,LS_tolerance)
				call copymatN_omp(matinv,blocks%butterflyUInv(i)%matrix,rank,dimension_mm)
				deallocate(matB,matC,matA,matinv)
			end do
...
end subroutine Resolving_Butterfly_LL_Nlog2N
