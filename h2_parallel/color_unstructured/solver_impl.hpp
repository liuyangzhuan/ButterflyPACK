...
void hierarchical_solve_unstructured(
    ...
    if (nrhs <= 0 || rhs.size() % static_cast<size_t>(nrhs) != 0) {
        throw std::invalid_argument(
            "hierarchical_solve_unstructured: invalid batched RHS dimensions");
    }
    ...
    for (int level = 0; level < num_levels; ++level) {
        ...
        if (level == leaf_level && global_idx != local_points) {
            throw std::runtime_error(
                "hierarchical_solve_unstructured: local RHS row count mismatch");
        }
    }
    ...
    for (int level = leaf_level; level >= 1; level--) {
        ...
        if (level >= 2 && tree_level.is_process_active &&
            ...
            for (int64_t local_idx : tree_level.boundary_id) {
                const int64_t morton_idx =
                    tree_level.local_boxes[static_cast<size_t>(local_idx)].morton_index;
                const int color_id = static_cast<int>(
                    morton_idx & (num_colors - 1));
                color_bins[static_cast<size_t>(color_id)].push_back(morton_idx);
            }
            for (int64_t local_idx : tree_level.interior_id) {
                const int64_t morton_idx =
                    tree_level.local_boxes[static_cast<size_t>(local_idx)].morton_index;
                const int color_id = static_cast<int>(
                    morton_idx & (num_colors - 1));
            ...
            for (int counter = 0; counter < static_cast<int>(color_bins.size()); ++counter) {
                ...
                if (color_list.empty()) {
                    ...
                    communication_total_forward += transport_and_apply_solve_updates_onehop(
                        tree, tree_level, level, pending_solve, solve_data, true);
                    continue;
                }
                ...
                // Distance-2 boxes in a Color wave can share an update target.
                // Accumulate into per-thread buffers and merge after the wave.
    ...
    for (int level = leaf_level; level >= 2; level--) {
        ...
        const int64_t occupied_box_count = static_cast<int64_t>(
            tree_level.boundary_id.size() + tree_level.interior_id.size());
        // Each occupied box owns its factor and right-hand side storage.
        #pragma omp parallel default(shared) if (occupied_box_count > 1)
        {
            int64_t local_diagonal_solves = 0;

            #pragma omp for schedule(static)
            for (int64_t occupied_slot = 0;
                 occupied_slot < occupied_box_count; ++occupied_slot) {
                const int64_t box_idx = occupied_slot <
                        static_cast<int64_t>(tree_level.boundary_id.size())
                    ? tree_level.boundary_id[static_cast<size_t>(occupied_slot)]
                    : tree_level.interior_id[static_cast<size_t>(
                        occupied_slot - tree_level.boundary_id.size())];
    ...
    for (int level = 1; level <= leaf_level; level++) {
        ...
        if (level >= 2 && tree_level.is_process_active &&
            ...
            for (int counter = static_cast<int>(color_bins.size()) - 1; counter >= 0; --counter) {
                ...
                get_data_start = clock::now();
                gather_boxes_solve(
                    tree, level, solve_data[level], true, false, false, true);
                communication_total_backward += (clock::now() - get_data_start);
                ...
                // Same-color boxes write distinct local solve records.
 ...
 */
template<typename CoordType, typename DataType>
void hierarchical_mul_unstructured(
    ParallelTree<CoordType, DataType>* tree,
    ...
    if (nrhs <= 0 || input_vec.size() % static_cast<size_t>(nrhs) != 0) {
        throw std::invalid_argument(
            "hierarchical_mul_unstructured: invalid batched input dimensions");
    }
    const int64_t local_points =
    ...
    for (int level = 0; level < num_levels; ++level) {
        ...
        if (level == leaf_level && global_idx != local_points) {
            throw std::runtime_error(
                "hierarchical_mul_unstructured: local input row count mismatch");
        }
    }
    ...
    for (int level = leaf_level; level >= 2; level--) {
        ...
        const int64_t occupied_box_count = static_cast<int64_t>(
            tree_level.boundary_id.size() + tree_level.interior_id.size());
        // Each occupied box owns its factor and vector storage.
        #pragma omp parallel default(shared) if (occupied_box_count > 1)
        {
            ...
            for (int counter = static_cast<int>(color_bins.size()) - 1; counter >= 0; --counter) {
                ...
                if (bins.empty()) {
                    ...
                    communication_total_backward += transport_and_apply_solve_updates_onehop(
                        tree, tree_level, level, pending_mul, solve_data, true);
                    continue;
                }
