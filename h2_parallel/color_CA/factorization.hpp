...
bool deferred_xnn_should_skip_owner_candidate_box(
    ...
    if (endpoint.box == nullptr ||
        (!endpoint.is_local && !(include_ghosts && endpoint.is_ghost))) {
        return true;
    }
...
bool deferred_xnn_should_skip_mirror_candidate_box(
    ...
    TreeLevel<CoordType, DataType>& level,
    bool include_ghosts = false) {
    return deferred_xnn_should_skip_owner_candidate_box(
...
bool deferred_xnn_first_box_owns_pair(
    ...
    if (use_CA_ownership) {
        return deferred_xnn_ownership_key(level, first_morton) <
            deferred_xnn_ownership_key(level, second_morton);
    }
    // Color has no ghost boxes. Both endpoints are local, so Morton order is
    // sufficient and agrees on the canonical half-pair.
...
void collect_owner_deferred_xnn_candidates_for_source_box(
    ...
    for (int64_t candidate_morton : source_box->one_hop) {
        ...
        if (include_ghosts) {
            if (endpoint.box != nullptr && !endpoint.is_assisting) {
                ...
                if (is_diagonal) {
                    ...
                    if (neighbor_endpoint.is_assisting) {
                        ...
                        if (assisting_key < candidate_key) {
                            std::ostringstream oss;
                            oss << "apply_owner_deferred_xnn_updates_for_candidate_box: "
...
void apply_symmetric_owner_deferred_xnn_updates_for_candidate_box(
    ...
    bool include_ghosts = false) {
    if (candidate_box == nullptr ||
        deferred_xnn_should_skip_mirror_candidate_box(
            candidate_box, level, include_ghosts)) {
        return;
    }
...
void gather_id_workspace(
    ...
    for (int64_t neighbor_morton : box->two_hop) {
        ...
        if (neighbor_box != nullptr) {
            ...
            if (neighbor_eliminated && !keep_full_boundary_neighbor) {
                n_neighbor = neighbor_box->skeleton_indices.size();
            } else {
                n_neighbor = neighbor_box->num_points;
            }
    ...
    for (size_t neighbor_idx = 0; neighbor_idx < box->two_hop.size(); ++neighbor_idx) {
        ...
        if (it != box->far_field_interaction_map.end()) {
            ...
            // Copy into workspace
            int64_t actual_rows = sliced_block.size() / workspace_cols;
            if (actual_rows != n_neighbor) {
                std::ostringstream oss;
                oss << "gather_id_workspace: stored far-field row mismatch"
                    << " box=" << box->morton_index
                    << " neighbor=" << neighbor_morton
                    << " expected_rows=" << n_neighbor
                    << " actual_rows=" << actual_rows
                    << " CA=" << use_CA_boundary_semantics;
                throw std::runtime_error(oss.str());
            }
            for (int64_t col = 0; col < workspace_cols; ++col) {
                for (int64_t row = 0; row < actual_rows; ++row) {
            ...
            if (neighbor_box != nullptr) {
                ...
                if (is_eliminated && !keep_full_boundary_neighbor) {
                    
                    // Use skeleton points only
                    actual_n_neighbor = neighbor_box->skeleton_indices.size();
                    if (actual_n_neighbor != n_neighbor) {
                        throw std::runtime_error(
                            "gather_id_workspace: local skeleton count mismatch");
                    }
                    ...
                    if (neighbor_box->point_indices.size() !=
                        static_cast<size_t>(n_neighbor)) {
                        throw std::runtime_error(
                            "gather_id_workspace: local full-point count mismatch");
                    }
                ...
                // CA assisting data is a full redundant copy. It remains full
                ...
                if (neighbor_eliminated) {
                    ...
                    if (neighbor_skel_indices.size() !=
                        static_cast<size_t>(n_neighbor)) {
                        throw std::runtime_error(
                            "gather_id_workspace: assisting skeleton count mismatch");
                    }
                    ...
                    if (req.indices.size() != static_cast<size_t>(n_neighbor)) {
                        throw std::runtime_error(
                            "gather_id_workspace: assisting full-point count mismatch");
                    }
                }
...
void slice_far_field_blocks(
    ...
    //
    // CA also keeps eliminated ghost boxes as writable mirror endpoints. Their
    // far-field views must therefore be normalized in the same wave as local
    // boxes, before deferred owner accumulation validates target dimensions.
    auto process_box = [&](BoxData<CoordType, DataType>& box) {
    ...
    for (auto& ghost_box : level.ghost_boxes) {
        if (level.eliminated_boxes.find(ghost_box.morton_index) ==
            level.eliminated_boxes.end()) {
            continue;
        }
        process_box(ghost_box);
    }
}
...
void compute_step_two_internal(
    ...
    if (!store && total_neighbor_points > 0) {
        ...
        for (size_t idx = 0; idx < box->one_hop.size(); ++idx) {
            ...
            const bool neighbor_is_writable =
                neighbor_local != nullptr ||
                (pending == nullptr && neighbor_box != nullptr);
            ...
            if (!neighbor_is_writable) {
                if (pending == nullptr) {
                    // CA may only have assisting data for this neighbor. Its
                    // owner applies the mirrored update during redundant work.
                    current_row += n_neighbor;
                    continue;
                }
            ...
            // Color writes only owned boxes; CA also writes its replicated ghosts.
            BoxData<CoordType, DataType>* neighbor_box_owned = neighbor_box;
        ...
        if (total_neighbor_points > 0 && !X_NN_full.empty()) {
            ...
            for (size_t j_idx = 0; j_idx < box->one_hop.size(); ++j_idx) {
                ...
                for (size_t i_idx = i_start; i_idx < box->one_hop.size(); ++i_idx) {
                    ...
                    // Color writes owner-local endpoints. CA, identified by a
                    // null pending-update sink, also writes replicated ghosts.
                    ...
                    const bool LG_owned_here = (LG_box != nullptr);
                    const bool RG_owned_here = (RG_box != nullptr);
...
void compute_and_modify(
    ...
    const bool use_CA_redundant_assisting = (pending_updates == nullptr);
        ...
        if (neighbor_box != nullptr) {
            ...
            if (assist_it != level.assisting_box_points_for_kernel_evaluation.end()) {
                if (!use_CA_redundant_assisting &&
                    ...
                    int64_t assist_idx = assist_it->second;
                    const auto& assist_box = level.assisting_boxes[assist_idx];
                    n_neighbor = assist_box.indices.size();
                    found_assist = true;
                }
    ...
    // Color-owned boxes use this cache during solve. CA ghost boxes derive the
    // same row partition from their gathered local/ghost solve data instead.
    if (level.is_box_on_process(box->morton_index)) {
        level.solve_neighbor_size[static_cast<size_t>(
            box->morton_index - level.local_morton_start)] =
                neighbor_point_counts;
    }
    ...
    if (total_neighbor_points > 0) {
        ...
        for (size_t idx = 0; idx < box->one_hop.size(); ++idx) {
            ...
            if (it != box->near_field_interaction_map.end()) {
                ...
                if (actual_rows != n_neighbor) {
                    std::ostringstream oss;
                    oss << "compute_and_modify: stored near-field row mismatch"
                        << " box=" << box->morton_index
                        << " neighbor=" << neighbor_morton
                        << " expected_rows=" << n_neighbor
                        << " actual_rows=" << actual_rows
                        << " CA=" << use_CA_redundant_assisting;
                    throw std::runtime_error(oss.str());
                }
                ...
                if (is_assisting) {
                    ...
                    if (n_neighbor != n_use) {
                        std::ostringstream oss;
                        oss << "compute_and_modify: assisting row mismatch"
                            << " box=" << box->morton_index
                            << " neighbor=" << neighbor_morton
                            << " expected_rows=" << n_neighbor
                            << " actual_rows=" << n_use
        ...
        if (current_row != total_neighbor_points) {
            throw std::runtime_error(
                "compute_and_modify: one-hop row count mismatch");
        }
...
void update_neighbor_slicing_for_level(
    TreeLevel<CoordType, DataType>& level,
    bool is_symmetric,
    bool use_received_assisting_skeletons = false) {
    ...
    auto slice_neighbor_dimension = [&](
        ...
        if (is_assisting) {
            ...
            if (use_received_assisting_skeletons ||
                level.eliminated_boxes.find(neighbor_morton) !=
                    level.eliminated_boxes.end()) {
                skeleton_indices = assisting_neighbor->skel_indices;
                has_skeleton = !skeleton_indices.empty();
            } else {
                has_skeleton = false;
            }
