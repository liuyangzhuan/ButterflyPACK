...
template<typename DataType>
size_t get_serialized_size_without_payload(
    const MatrixStorage<DataType>& mat);
...
template<typename DataType>
char* serialize_without_payload(
    const MatrixStorage<DataType>& mat, char* buffer);
...
template<typename DataType>
size_t get_serialized_size_without_payload(
    const MatrixStorage<DataType>&) {
    size_t size = 0;
    size += sizeof(int64_t);  // rows
    size += sizeof(int64_t);  // cols
    size += sizeof(int64_t);  // lda
    size += sizeof(typename MatrixStorage<DataType>::Format);  // format
    size += sizeof(size_t);   // data.size() = 0
    return size;
}
...
char* serialize_without_payload(
    ...
    std::memcpy(
        buffer, &mat.format,
        sizeof(typename MatrixStorage<DataType>::Format));
    buffer += sizeof(typename MatrixStorage<DataType>::Format);
...
template<typename DataType>
char* serialize(const ModifiedBlock<DataType>& block, char* buffer) {
    return serialize(block, buffer, false);
}
...
char* serialize(
    ...
    char* buffer,
    bool omit_A_NS_payload) {
    const bool reconstruct_A_NS_from_symmetric_pair =
        block.reconstruct_A_NS_from_symmetric_pair || omit_A_NS_payload;

    std::memcpy(buffer, &block.neighbor_morton, sizeof(int64_t));
    buffer += sizeof(int64_t);

    std::memcpy(
        buffer, &reconstruct_A_NS_from_symmetric_pair, sizeof(bool));
    buffer += sizeof(bool);
...
const char* deserialize(ModifiedBlock<DataType>& block, const char* buffer) {
    ...
    std::memcpy(
        &block.reconstruct_A_NS_from_symmetric_pair, buffer, sizeof(bool));
    buffer += sizeof(bool);
...
template<typename CoordType, typename DataType, typename OwnerRankFn>
bool should_omit_symmetric_halo_A_NS_payload(
    const BoxData<CoordType, DataType>& box,
    const ModifiedBlock<DataType>& block,
    const std::unordered_set<int64_t>& receiver_full_requested_ghosts,
    int receiver_rank,
    OwnerRankFn&& owner_rank_for_morton,
    bool enable_symmetric_pair_pruning) {
    if (!enable_symmetric_pair_pruning ||
        block.neighbor_morton < 0 ||
        box.morton_index <= block.neighbor_morton) {
        return false;
    }
    ...
    const bool neighbor_is_local_on_receiver =
        owner_rank_for_morton(block.neighbor_morton) == receiver_rank;
    const bool neighbor_is_requested_as_ghost =
        receiver_full_requested_ghosts.find(block.neighbor_morton) !=
        receiver_full_requested_ghosts.end();
    return neighbor_is_local_on_receiver || neighbor_is_requested_as_ghost;
}

template<typename CoordType, typename DataType, typename OwnerRankFn>
size_t get_serialized_size_for_CA_ghost_halo_box(
    const BoxData<CoordType, DataType>& box,
    const std::unordered_set<int64_t>& receiver_full_requested_ghosts,
    int receiver_rank,
    OwnerRankFn&& owner_rank_for_morton,
    ...
    if (!enable_symmetric_pair_pruning) return size;

    auto subtract_omitted_payload = [&](const auto& blocks) {
        for (const auto& block : blocks) {
            if (should_omit_symmetric_halo_A_NS_payload(
    ...
    subtract_omitted_payload(box.near_field_modified_interactions);
    subtract_omitted_payload(box.far_field_modified_interactions);
    return size;
}

template<typename CoordType, typename DataType, typename OwnerRankFn>
char* serialize_CA_ghost_halo_box(
    const BoxData<CoordType, DataType>& box,
    const std::unordered_set<int64_t>& receiver_full_requested_ghosts,
    int receiver_rank,
    OwnerRankFn&& owner_rank_for_morton,
    ...
    auto serialize_map = [&ptr](const auto& map) {
        const size_t map_size = map.size();
        std::memcpy(ptr, &map_size, sizeof(size_t));
        ptr += sizeof(size_t);
    ...
    serialize_map(box.near_field_interaction_map);
    serialize_map(box.near_field_interaction_map_nonsymmetry);
    serialize_map(box.far_field_interaction_map);
    serialize_map(box.far_field_interaction_map_nonsymmetry);

    const size_t near_blocks_size =
        box.near_field_modified_interactions.size();
    std::memcpy(ptr, &near_blocks_size, sizeof(size_t));
    ptr += sizeof(size_t);
    for (const auto& block : box.near_field_modified_interactions) {
        const bool omit_A_NS_payload =
            should_omit_symmetric_halo_A_NS_payload(
                box, block, receiver_full_requested_ghosts,
                receiver_rank, owner_rank_for_morton,
                enable_symmetric_pair_pruning);
        ptr = serialize(block, ptr, omit_A_NS_payload);
    }

    const size_t far_blocks_size =
        box.far_field_modified_interactions.size();
    std::memcpy(ptr, &far_blocks_size, sizeof(size_t));
    ptr += sizeof(size_t);
    for (const auto& block : box.far_field_modified_interactions) {
        const bool omit_A_NS_payload =
...
struct CARequestPlan {
    std::vector<std::vector<int64_t>> outgoing;
    std::vector<std::vector<int64_t>> incoming;
...
CARequestPlan make_CA_request_plan(
    ...
    for (int comm_rank = 0; comm_rank < comm_size; ++comm_rank) {
        comm_rank_by_global[global_rank_by_comm[static_cast<size_t>(comm_rank)]] = comm_rank;
    }

    CARequestPlan plan;
    plan.outgoing.resize(static_cast<size_t>(comm_size));
    plan.incoming.resize(static_cast<size_t>(comm_size));
    plan.global_rank_by_comm = global_rank_by_comm;

    std::vector<uint64_t> mortons;
    mortons.reserve(requested_mortons.size());
    for (int64_t morton : requested_mortons) {
    ...
    for (size_t idx = 0; idx < requested_mortons.size(); ++idx) {
        const int owner_global = lvl.morton_to_rank.at(
            static_cast<int>(owner_regions[idx]));
        if (owner_global == tree->mpi_rank) {
            continue;
        }
        auto comm_it = comm_rank_by_global.find(owner_global);
        if (comm_it == comm_rank_by_global.end()) {
            throw std::runtime_error(
                "make_CA_request_plan: owner rank is absent from the active communicator");
        }
        plan.outgoing[static_cast<size_t>(comm_it->second)].push_back(
            requested_mortons[idx]);
    }

    std::vector<int> send_counts(static_cast<size_t>(comm_size), 0);
    std::vector<int> recv_counts(static_cast<size_t>(comm_size), 0);
    std::vector<int> send_displs(static_cast<size_t>(comm_size), 0);
    std::vector<int> recv_displs(static_cast<size_t>(comm_size), 0);
    for (int peer = 0; peer < comm_size; ++peer) {
        auto& peer_requests = plan.outgoing[static_cast<size_t>(peer)];
        std::sort(peer_requests.begin(), peer_requests.end());
        peer_requests.erase(
            std::unique(peer_requests.begin(), peer_requests.end()),
            peer_requests.end());
        send_counts[static_cast<size_t>(peer)] =
            static_cast<int>(peer_requests.size());
    }
    ...
    for (int peer = 1; peer < comm_size; ++peer) {
        send_displs[static_cast<size_t>(peer)] =
            send_displs[static_cast<size_t>(peer - 1)] +
            send_counts[static_cast<size_t>(peer - 1)];
        recv_displs[static_cast<size_t>(peer)] =
            recv_displs[static_cast<size_t>(peer - 1)] +
            recv_counts[static_cast<size_t>(peer - 1)];
    }

    const int total_send = comm_size == 0 ? 0 :
        send_displs.back() + send_counts.back();
    const int total_recv = comm_size == 0 ? 0 :
        recv_displs.back() + recv_counts.back();
    std::vector<int64_t> send_buffer(static_cast<size_t>(total_send));
    std::vector<int64_t> recv_buffer(static_cast<size_t>(total_recv));
    for (int peer = 0; peer < comm_size; ++peer) {
        std::copy(
            plan.outgoing[static_cast<size_t>(peer)].begin(),
            plan.outgoing[static_cast<size_t>(peer)].end(),
            send_buffer.begin() + send_displs[static_cast<size_t>(peer)]);
    }

    MPI_Alltoallv(
        send_buffer.empty() ? nullptr : send_buffer.data(),
        send_counts.data(), send_displs.data(), MPI_INT64_T,
        recv_buffer.empty() ? nullptr : recv_buffer.data(),
        recv_counts.data(), recv_displs.data(), MPI_INT64_T,
        level_comm);

    for (int peer = 0; peer < comm_size; ++peer) {
        const int begin = recv_displs[static_cast<size_t>(peer)];
        const int count = recv_counts[static_cast<size_t>(peer)];
        plan.incoming[static_cast<size_t>(peer)].assign(
            recv_buffer.begin() + begin,
            recv_buffer.begin() + begin + count);
    }
    return plan;
}

template<typename SizeFunction, typename SerializeFunction, typename ConsumeFunction>
void exchange_CA_requested_payloads(
    const CARequestPlan& plan,
    MPI_Comm level_comm,
    int payload_tag,
    ...
    std::vector<uint64_t> send_sizes(static_cast<size_t>(comm_size), 0);
    std::vector<uint64_t> recv_sizes(static_cast<size_t>(comm_size), 0);
    std::vector<std::vector<char>> send_buffers(static_cast<size_t>(comm_size));
    std::vector<std::vector<char>> recv_buffers(static_cast<size_t>(comm_size));

    for (int peer = 0; peer < comm_size; ++peer) {
        size_t bytes = 0;
        for (int64_t morton : plan.incoming[static_cast<size_t>(peer)]) {
            bytes += sizeof(size_t) + payload_size(peer, morton);
        }
        send_sizes[static_cast<size_t>(peer)] = static_cast<uint64_t>(bytes);
        auto& buffer = send_buffers[static_cast<size_t>(peer)];
        buffer.resize(bytes);
        char* ptr = buffer.data();
        for (int64_t morton : plan.incoming[static_cast<size_t>(peer)]) {
            const size_t item_size = payload_size(peer, morton);
            std::memcpy(ptr, &item_size, sizeof(size_t));
            ptr += sizeof(size_t);
            char* item_end = serialize_payload(peer, morton, ptr);
            if (item_end != ptr + item_size) {
                throw std::runtime_error(
                    "exchange_CA_requested_payloads: serialized size mismatch");
            }
            ptr = item_end;
        }
    }
    ...
    std::vector<MPI_Request> requests;
    for (int peer = 0; peer < comm_size; ++peer) {
        const size_t bytes = static_cast<size_t>(recv_sizes[static_cast<size_t>(peer)]);
        if (bytes == 0) continue;
        recv_buffers[static_cast<size_t>(peer)].resize(bytes);
        const int err = MPI_Irecv_large(
            recv_buffers[static_cast<size_t>(peer)].data(), bytes, MPI_CHAR,
            peer, payload_tag, level_comm, requests);
        if (err != MPI_SUCCESS) {
    ...
    for (int peer = 0; peer < comm_size; ++peer) {
        ...
        if (bytes == 0) continue;
        const int err = MPI_Isend_large(
            send_buffers[static_cast<size_t>(peer)].data(), bytes, MPI_CHAR,
            peer, payload_tag, level_comm, requests);
        if (err != MPI_SUCCESS) {
            throw std::runtime_error("CA payload send failed");
        }
    }
    if (!requests.empty()) {
        MPI_Waitall(
    ...
    for (int peer = 0; peer < comm_size; ++peer) {
        const auto& buffer = recv_buffers[static_cast<size_t>(peer)];
        if (buffer.empty()) {
            if (!plan.outgoing[static_cast<size_t>(peer)].empty()) {
                throw std::runtime_error("CA payload is missing");
            }
            continue;
        }
        const char* ptr = buffer.data();
        const char* end = ptr + buffer.size();
        for (int64_t morton : plan.outgoing[static_cast<size_t>(peer)]) {
            if (static_cast<size_t>(end - ptr) < sizeof(size_t)) {
                throw std::runtime_error("CA payload header is truncated");
            }
            size_t item_size = 0;
            std::memcpy(&item_size, ptr, sizeof(size_t));
            ptr += sizeof(size_t);
            if (static_cast<size_t>(end - ptr) < item_size) {
                throw std::runtime_error("CA payload is truncated");
            }
            const char* item_end = consume_payload(peer, morton, ptr);
            if (item_end != ptr + item_size) {
                throw std::runtime_error(
                    "exchange_CA_requested_payloads: deserialized size mismatch");
            }
            ptr = item_end;
        }
        if (ptr != end) {
...
inline std::vector<std::unordered_set<int64_t>>
exchange_CA_full_requested_ghost_sets(
    const CARequestPlan& plan,
    const std::vector<int64_t>& requested_mortons,
    MPI_Comm level_comm) {
    int comm_size = 0;
    ...
    std::vector<int64_t> full_request_set = requested_mortons;
    std::sort(full_request_set.begin(), full_request_set.end());
    full_request_set.erase(
        std::unique(full_request_set.begin(), full_request_set.end()),
        full_request_set.end());
    if (full_request_set.size() > static_cast<size_t>(INT_MAX)) {
        throw std::runtime_error("CA full ghost request set exceeds MPI int count");
    }
    ...
    for (int peer = 0; peer < comm_size; ++peer) {
        if (!plan.outgoing[static_cast<size_t>(peer)].empty()) {
            send_counts[static_cast<size_t>(peer)] =
                static_cast<int>(full_request_set.size());
        }
    }
    ...
    for (int peer = 0; peer < comm_size; ++peer) {
        if (total_send > INT_MAX || total_recv > INT_MAX) {
            throw std::runtime_error("CA full ghost request exchange exceeds MPI int displacement");
        }
        send_displs[static_cast<size_t>(peer)] = static_cast<int>(total_send);
        recv_displs[static_cast<size_t>(peer)] = static_cast<int>(total_recv);
        total_send += send_counts[static_cast<size_t>(peer)];
        total_recv += recv_counts[static_cast<size_t>(peer)];
    }
    if (total_send > INT_MAX || total_recv > INT_MAX) {
        throw std::runtime_error("CA full ghost request exchange exceeds MPI int buffer size");
    }

    std::vector<int64_t> send_buffer(static_cast<size_t>(total_send));
    std::vector<int64_t> recv_buffer(static_cast<size_t>(total_recv));
    for (int peer = 0; peer < comm_size; ++peer) {
        if (send_counts[static_cast<size_t>(peer)] == 0) continue;
        std::copy(
            full_request_set.begin(), full_request_set.end(),
            send_buffer.begin() + send_displs[static_cast<size_t>(peer)]);
    }
    ...
    std::vector<std::unordered_set<int64_t>> received_sets(
        static_cast<size_t>(comm_size));
    for (int peer = 0; peer < comm_size; ++peer) {
        const auto begin = recv_buffer.begin() +
            recv_displs[static_cast<size_t>(peer)];
        const auto end = begin + recv_counts[static_cast<size_t>(peer)];
        received_sets[static_cast<size_t>(peer)].insert(begin, end);
    }
    return received_sets;
}

template<typename CoordType, typename DataType>
void gather_CA_assisting_boxes_factorization(
    ParallelTree<CoordType, DataType>* tree,
    int level,
    MPI_Comm level_comm,
    int payload_tag,
    ...
    for (const auto& entry : lvl.assisting_box_points_for_kernel_evaluation) {
    ...
    auto make_request = [&](int64_t morton) {
        const auto* box = lvl.find_local_box(morton);
        if (box == nullptr) {
            throw std::runtime_error(
                "CA assisting request references a nonlocal source box");
        }
        PointDataRequest<CoordType> request;
        request.morton_index = morton;
        request.source_rank = tree->mpi_rank;
        request.coords = box->point_coords;
        request.indices = box->point_indices;
        request.skel_indices = box->skeleton_indices;
        request.on_boundary = box->on_boundary;
        return request;
    };
    ...
    exchange_CA_requested_payloads(
        ...
        [&](int, int64_t morton) {
            return get_serialized_size(make_request(morton));
        },
        [&](int, int64_t morton, char* ptr) {
            return serialize(make_request(morton), ptr);
        },
        [&](int, int64_t morton, const char* ptr) {
            PointDataRequest<CoordType> request;
            const char* end = deserialize(request, ptr);
            if (request.morton_index != morton) {
                throw std::runtime_error("CA assisting payload Morton mismatch");
            }
...
template<typename CoordType, typename DataType>
void reconstruct_CA_symmetric_ghost_halo(
    TreeLevel<CoordType, DataType>& level) {
    auto reconstruct_blocks = [&](BoxData<CoordType, DataType>& target_box,
                                  std::vector<ModifiedBlock<DataType>>& target_blocks,
                                  bool is_near) {
        for (auto& target_block : target_blocks) {
            if (!target_block.reconstruct_A_NS_from_symmetric_pair) continue;

            BoxData<CoordType, DataType>* source_box =
                level.find_local_box(target_block.neighbor_morton);
            if (source_box == nullptr) {
                source_box = level.find_ghost_box(target_block.neighbor_morton);
            }
            if (source_box == nullptr) {
                throw std::runtime_error(
                    "reconstruct_CA_symmetric_ghost_halo: source box missing");
            }

            auto& source_map = is_near
                ? source_box->near_field_interaction_map
                : source_box->far_field_interaction_map;
            auto& source_blocks = is_near
                ? source_box->near_field_modified_interactions
                : source_box->far_field_modified_interactions;
            const auto source_it = source_map.find(target_box.morton_index);
...
void gather_CA_factorization_data(
    ...
    exchange_CA_requested_payloads(
        ...
        [&](int peer, int64_t morton) {
            const auto* box = lvl.find_local_box(morton);
            if (box == nullptr) {
                throw std::runtime_error("CA ghost request references a nonlocal source box");
            }
            return get_serialized_size_for_CA_ghost_halo_box(
                *box, peer_full_request_sets[static_cast<size_t>(peer)],
                ghost_plan.global_rank_by_comm[static_cast<size_t>(peer)],
                owner_rank_for_morton, enable_symmetric_pair_pruning);
        },
        [&](int peer, int64_t morton, char* ptr) {
            const auto* box = lvl.find_local_box(morton);
            if (box == nullptr) {
                throw std::runtime_error("CA ghost request references a nonlocal source box");
            }
            return serialize_CA_ghost_halo_box(
                *box, peer_full_request_sets[static_cast<size_t>(peer)],
                ghost_plan.global_rank_by_comm[static_cast<size_t>(peer)],
                owner_rank_for_morton, enable_symmetric_pair_pruning, ptr);
        },
        [&](int, int64_t morton, const char* ptr) {
            BoxData<CoordType, DataType> box;
            const char* end = deserialize(box, ptr);
            if (box.morton_index != morton) {
                throw std::runtime_error("CA ghost payload Morton mismatch");
            }
...
void gather_CA_boxes_solve(
    ...
    auto& lvl = tree->levels[level];
    if (!lvl.is_process_active || level_comm == MPI_COMM_NULL) return;

    std::vector<int64_t> requested_mortons;
    requested_mortons.reserve(
        lvl.ghost_and_assisting_box_points_for_solve_map.size());
    for (const auto& [morton, index] :
         lvl.ghost_and_assisting_box_points_for_solve_map) {
        if (!assist_only || !lvl.is_ghost_solve[static_cast<size_t>(index)]) {
            requested_mortons.push_back(morton);
        }
    }
    std::sort(requested_mortons.begin(), requested_mortons.end());

    const CARequestPlan plan = make_CA_request_plan(
        tree, level, requested_mortons, level_comm);
    lvl.ghost_and_assisting_boxes_for_solve.resize(
        lvl.ghost_and_assisting_box_points_for_solve_map.size());

    auto make_request = [&](int64_t morton) {
        const auto* box = lvl.find_local_box(morton);
        const int64_t local_idx = morton - lvl.local_morton_start;
        if (box == nullptr || local_idx < 0 ||
            local_idx >= static_cast<int64_t>(local_solve_data.size())) {
            throw std::runtime_error(
                "CA solve request references unavailable local Morton box " +
                std::to_string(morton));
        }

        SolveDataRequest<CoordType, DataType> request;
        request.morton_index = morton;
        request.source_rank = tree->mpi_rank;
        request.copy_minimal_from_box(
            *box, local_solve_data[static_cast<size_t>(local_idx)]);
        return request;
    };
    ...
    exchange_CA_requested_payloads(
        ...
        [&](int, int64_t morton, const char* ptr) {
            ...
            const char* end = deserialize(request, ptr);
            if (request.morton_index != morton) {
                throw std::runtime_error("CA solve payload Morton mismatch");
            }

            const size_t index = static_cast<size_t>(
                lvl.ghost_and_assisting_box_points_for_solve_map.at(morton));
            ...
            cached.skeleton_indices = std::move(request.skeleton_indices);
            return end;
        });
}
...
void gather_boxes_solve(
    ...
    auto should_transfer_request = [&](const RequestInfo& req_info) {
        return !assist_only || !req_info.is_ghost;
    };
    ...
    auto get_serialized_request_size_for_box =
        [&](const RequestInfo& req_info, int64_t local_idx) -> size_t {
            ...
            if (uses_factorization_data(req_info)) {
                size += vector_serialized_size(box.one_hop);
                size += vector_serialized_size(box.use_full_set);
                size += vector_serialized_size(box.X_RR_pivots);
                size += get_serialized_size(box.interpolation_matrix);
                size += get_serialized_size(box.X_RR);
                size += get_serialized_size(box.X_RS);
                size += get_serialized_size(box.X_SR);
    ...
    auto populate_solve_request =
        [&](SolveDataRequest<CoordType, DataType>& solve_req,
            ...
            if (uses_factorization_data(req_info)) {
                solve_req.copy_factorization_from_box(
    ...
    for (const auto& [morton, idx] : lvl.ghost_and_assisting_box_points_for_solve_map) {
        ...
        if (owner_rank != rank) {
            ...
            if (should_transfer_request(req_info)) {
