...
struct ParallelTree {
    ...
    int64_t minimum_CA_boxes_per_active_process() const {
        return (dimension == 2) ? int64_t{25} : int64_t{343};
    }

    int64_t boxes_per_active_process(int level) const {
        if (level < 0 || level >= num_levels ||
            levels[level].num_active_processes <= 0) {
            return 0;
        }

        const int64_t global_boxes = levels[level].num_boxes_global;
        if (global_boxes <= 0) {
            return 0;
        }
        return global_boxes / levels[level].num_active_processes;
    }
    ...
    bool level_supports_CA_geometry(int level) const {
        ...
        if (levels[level].num_active_processes <= 0) {
            return false;
        }
        if (levels[level].num_active_processes == 1) {
            return true;
        }
        return boxes_per_active_process(level) >=
               minimum_CA_boxes_per_active_process();
    }

    bool level_uses_CA(int level) const {
        return level_requests_CA(level) && level_supports_CA_geometry(level);
    }
