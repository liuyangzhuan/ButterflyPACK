...
resolve_solve_data_for_morton(
    TreeLevel<CoordType, DataType>& level,
    std::vector<SolveDataRequest<CoordType, DataType>>& level_solve_data,
    int64_t morton,
    bool local_or_ghost_only = false) {
    if (morton >= level.local_morton_start &&
        morton <= level.local_morton_end) {
    ...
    if (local_or_ghost_only && !level.is_ghost_solve[it->second]) {
        return nullptr;
    }
...
template<typename CoordType, typename DataType>
static inline std::vector<int64_t> solve_neighbor_sizes_for_box(
    TreeLevel<CoordType, DataType>& level,
    std::vector<SolveDataRequest<CoordType, DataType>>& level_solve_data,
    const SolveDataRequest<CoordType, DataType>& solve_data,
    const std::vector<int64_t>& one_hop,
    const std::vector<int64_t>& use_full_set,
    bool is_ghost) {
    if (!is_ghost) {
        const int64_t local_offset =
            solve_data.morton_index - level.local_morton_start;
        if (local_offset >= 0 &&
            local_offset < static_cast<int64_t>(level.solve_neighbor_size.size())) {
            const auto& cached =
                level.solve_neighbor_size[static_cast<size_t>(local_offset)];
            if (cached.size() == one_hop.size()) {
                return cached;
            }
        }
    }

    if (use_full_set.size() != one_hop.size()) {
        throw std::runtime_error(
            "solve_neighbor_sizes_for_box: use_full_set/one_hop size mismatch");
    }

    std::vector<int64_t> sizes;
    sizes.reserve(one_hop.size());
    for (size_t i = 0; i < one_hop.size(); ++i) {
        auto* neighbor = resolve_solve_data_for_morton(
            level, level_solve_data, one_hop[i]);
        if (neighbor == nullptr) {
            throw std::runtime_error(
...
template<typename CoordType, typename DataType>
static inline void apply_pending_solve_updates_local_or_ghost(
    TreeLevel<CoordType, DataType>& level,
    std::vector<SolveDataRequest<CoordType, DataType>>& level_solve_data,
    const PendingSolveUpdates<DataType>& pending) {
    ...
    bool local_or_ghost_targets_only = false)
{
    ...
    if (is_ghost) {
        auto* box = level.find_ghost_box(solve_data.morton_index);
        if (box != nullptr) {
            T = &box->interpolation_matrix;
            X_SR = &box->X_SR;
            X_NR = &box->X_NR;
    ...
    for (size_t neighbor_idx = 0; neighbor_idx < one_hop->size(); ++neighbor_idx) {
        ...
        if (is_local_morton(neighbor_morton)) {
            ...
            if (!local_or_ghost_targets_only || neighbor_target != nullptr) {
                // Color transports remote updates. CA applies only updates whose
                // target is local or represented by a writable ghost.
                if (use_full) {
                    accumulate_replace_then_add(
                        pending_updates.full_updates,
