...
export BPACK_PYTHON_LIB_PATH=$ROOT_DIR/build/lib/
...
10) mpirun --allow-run-as-root --oversubscribe "-n" "5" "$EXAMPLE_FOLDER/full" "-option" "--less_adapt" "0" "--errsol" "1";; # test HODLR with LR for reading a KRR matrix from a file 
11) mpirun --allow-run-as-root --oversubscribe "-n" "2" "$EXAMPLE_FOLDER/frontal" "-quant" "--data_dir" "$DATA_FOLDER/FULLMAT_DATA/Frontal_poisson_50.mat" "--nunk" "50" "--explicitflag" "0" "-option" "--lrlevel" "0" "--tol_comp" "1d-1" "--nmin_leaf" "100" "--errsol" "1" # test sampling-based construction of HODLR with LR for an 3D poisson frontal matrix stored in file
   mpirun --allow-run-as-root --oversubscribe "-n" "2" "$EXAMPLE_FOLDER/frontal" "-quant" "--data_dir" "$DATA_FOLDER/FULLMAT_DATA/Frontal_poisson_50.mat" "--nunk" "50" "--explicitflag" "0" "-option" "--lrlevel" "100" "--tol_comp" "1d-1" "--nmin_leaf" "100" "--errsol" "1" # test sampling-based construction of HODLR with BF for an 3D poisson frontal matrix stored in file
   mpirun --allow-run-as-root --oversubscribe "-n" "2" "$EXAMPLE_FOLDER/frontal" "-quant" "--data_dir" "$DATA_FOLDER/FULLMAT_DATA/Frontal_poisson_50.mat" "--nunk" "50" "--explicitflag" "1" "-option" "--lrlevel" "100" "--tol_comp" "1d-1" "--nmin_leaf" "100" "--errsol" "1";; # test entry-evaluation-based construction of HODLR with BF for an 3D poisson frontal matrix stored in file   
...
19) python3 $DATA_FOLDER/Test_python_allranks.py -option --xyzsort 1 --tol_comp 1e-6 --lrlevel 0 --reclr_leaf 5 --nmin_leaf 128 --errsol 1 # all-rank python launch mode without MPI  
    python3 ${BPACK_PYTHON_LIB_PATH}/dPy_BPACK_worker.py -option --xyzsort 1 --tol_comp 1e-10 --lrlevel 0 --reclr_leaf 5 --nmin_leaf 128 --errsol 1 & # master-worker python launch mode without MPI  
    python3 $DATA_FOLDER/Test_python_master.py 
    python3 -c "from dPy_BPACK_wrapper import *; bpack_terminate()";; 
20) mpirun --allow-run-as-root --oversubscribe "-n" "2" python3 $DATA_FOLDER/Test_python_allranks.py -option --xyzsort 1 --tol_comp 1e-6 --lrlevel 0 --reclr_leaf 5 --nmin_leaf 128 --errsol 1 # all-rank python launch mode without MPI  
